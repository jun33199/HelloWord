...
public class JmsbaHjMainHandler extends ActionHandler {
	MyLogger myLogger =new MyLogger(JmsbaHjMainHandler.class);
	...
	private StringBuffer doShow(ClientMessage msg) throws FrameException{
		...
		try {
			...
			for (sf_dm_jmbasxdm sf_dm_jmbasxdm : basxdmList) {
				...
				if(sf_dm_jmbasxdm.getHjbs().equals("0")){
			...
			if("INNER".equals(deploy_environment)){
				...
				resVo.setInZqFlag(SbzlAccess.checkinZq("JMSBAHSQJ2014")?"Y":"N");
			}
	...
	private StringBuffer doAdd(ClientMessage msg) throws FrameException{
		...
		//jsjdm band baqsxdm qh=2
		//SF_JL_QYSDSJMSBAJL_2014
