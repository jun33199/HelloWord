package com.lscdz.qysds.application.qysdsjb2014.hdzsjb.helper;
 ...
 */
public class HdzsjbHelper extends QysdsjbBaseHelper{
	...
	public void save(QysdsjbBaseVo qysdsjbBaseVo) throws FrameException{
		qysdsjbBaseVo.setZsfsdm("01");//核定征收
		super.save(qysdsjbBaseVo);	
	}
	@Override
	public void query(QysdsjbBaseVo qysdsjbBaseVo) throws FrameException{
		HdzsjbVo hdzsjbVo=(HdzsjbVo)qysdsjbBaseVo;
		Connection conn = null;
		try {		
		// 获取税款所属季度
		String jd = QysdsBaseUtil.preQuarter(DateUtils.getDateTime(hdzsjbVo.getSkssjsrq()));
		String nd = hdzsjbVo.getSkssksrq().substring(0, 4);
		// 设置季度
		hdzsjbVo.setQh(jd);
		// 设置年度
		hdzsjbVo.setSknd(nd);
...
//		conn = ConnFactoryTest.getConnect();
		hdzsjbVo = (HdzsjbVo) QysdsBaseUtil.queryDjxxByInterfaceDJ(conn, hdzsjbVo);
		// 获得企业登记基本信息
		...
		//获取是否是新开户  是：Y 否：N 
		hdzsjbVo.setSfxkh(qysdsUtil2014.getSfxkh(hdzsjbVo.getSknd(), djsj, hdzsjbVo.getQh()));
		//获取税款所属期所在年度上一年度征收方式
		hdzsjbVo.setSyndZsfsdm(qysdsUtil2014.getSyndZsfsDm(hdzsjbVo.getJsjdm(), hdzsjbVo.getSknd(),hdzsjbVo.getQh()));
		//获取税款所属期所在年度上一年度的汇算清缴申报信息 主表行9、25 附表五45、46、47
		Map map=qysdsUtil2014.getSyndHsqjSbxx(hdzsjbVo.getSyndZsfsdm(),hdzsjbVo.getJsjdm(), hdzsjbVo.getSknd(),hdzsjbVo.getQh());
		hdzsjbVo.setSyndZbh6(map.get("syndZbh6").toString());
		hdzsjbVo.setSyndZbh25(map.get("syndZbh25").toString());
		hdzsjbVo.setSyndFb5jyjg(map.get("syndFb5jyjg").toString());
		this.getHdxx(hdzsjbVo);
		/* 征收方式 */
		String zsfs = hdzsjbVo.getZsfsdm();

		System.out.println(hdzsjbVo.getJsjdm() + "的征收方式代码：" + zsfs);
		...
		hdzsjbVo.setReportData(super.queryReportData(this.getReportVo(hdzsjbVo)));
	}
	@Override
	protected ReportVo getReportVo(QysdsjbBaseVo qysdsjbBaseVo) {
		...
		reportVo.setTid(QysdsJb2014Contant.TABLE_ID_HDZSSDSJB_2008);
		reportVo.setTname(QysdsJb2014Contant.TABLE_NAME_HDZSSDSJB_2008);
	 ...
	 */
	private void getHdxx(HdzsjbVo form) throws FrameException {
		Connection conn=null;
