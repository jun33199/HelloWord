...
public class FindObjInList
{
     ...
     * @throws BaseException 异常情况 比如参数不合法
     */
    public static Object find(List source,
                              Class objClass,
                              List member,
                              List value,
            ...
            if(tagField)
            {
               //循环确定调用指定method后得到是否与给定的value匹配
                for(int m=0; m<methodList.size(); m++)
                {
                    try
                    {
                        paramobj = ( (Method)methodList.get(m)).invoke(voSource, null);
                        if(paramobj == null)
                        {
     ...
     * @throws BaseException 异常情况 比如参数不合法
     */
    public static List findAll(List source, Class objClass, List member, List value)
        throws BaseException
     ...
     * @throws BaseException 异常情况 比如参数不合法
     */
    public static Object findOne(List source, Class objClass, List member, List value)
        throws BaseException
     ...
     */
    public static List splitListByParam(List source, Class objClass, List member)
        throws BaseException
    {
        ...
        while(source.size() > 0)
        {
            tmpValueList = getValueList(source.get(0), objClass, fieldList, methodList);
            //find操作会删除source中元素
            resultList.add(find(source, objClass, member, tmpValueList, FINDALL, true));
        }
        return resultList;
    }
     ...
     */
    private static List getValueList(Object obj, Class objClass, List fieldList,
                                     List methodList) throws BaseException
    {
        ...
        for(int i = 0; i < fieldList.size(); i++)
        {
            ...
            {
                throw new SystemException("找不到指定的成员");
            }
        ...
        for(int j = 0; j < methodList.size(); j++)
        {
            ...
            {
                //发生异常就认为null
                throw new SystemException("找不到指定的方法");
            }
            valueList.add(tmpValue);
        }
        return valueList;
     ...
     */
    private static Map paramCheck(List source ,Class objClass,List member,List value)
        throws BaseException
    {
        //临时变量
        Field tmpField = null;
        ...
        {
            throw new SystemException("所有参数都不能为null");
        }

        //成员数量必须于值数量一致
        if(member.size() != value.size())
        {
            throw new SystemException("(member.size:" + member.size() + " != " +
