...
/**
 * 
 * Description : 查帐征收企业所得税年报Action
 * @author zhangj
 * @version 2014-9-17 上午11:54:52
 */
public class CzzsfzjgNbAction extends DcBaseAction {

	private String errorMessage = "";

	public CzzsfzjgNbAction() {

	}

	protected boolean validate(CzzssdsNbVO qynb) {
		if (!qynb.getYwlx().equals(QysdsNbConstant2014.CA_YWLXDM_CZZSSDSNB_2014)) {
	...
	public String doShow(HttpServletRequest request,
		...
		try {
			...
			pDataMap.put(QysdsksMapConstant.STRING_KEY_BBLX, QysdsNbConstant2014.TABLE_ID_CZZSSDSNB_2014);
			...
			vo.setUserData(ud);
			// 调用后台查询数据
			qysdsnbbo = (CzzsfzjgNbBO) ShenbaoProxy.getInstance().process(vo);
			qysdsnbvo = qysdsUtil.Czzsconvert2XMLVO(qysdsnbbo, djJbsj, QysdsNbConstant2014.TABLE_ID_CZZSSDSNB_2014);
			...
			request.getSession(false).setAttribute("REQ_KEY_QYSBSNB_2014", qysdsnbvo);
	...
	public String doSave(HttpServletRequest request,HttpServletResponse response) throws BaseException 
	{
		...
		try {
			...
			if("1".equals(nsfs)){//如果申报表类型为汇总缴纳
				request.setAttribute(QysdsNbConstant2014.JUMP_FLAG_NAME,"1");//表明保存成功,建议跳转
...
//			return CAConstants.SUCCESSSB;
	...
	public String doDelete(HttpServletRequest request,HttpServletResponse response) throws BaseException 
	{
		...
		try {
			...
			SWDJJBSJ djJbsj = (SWDJJBSJ) FriendHelper.getSWDJJBSJ(request);
			// 更新form中的数据
			CzzsfzjgNbBO qysdsnd = qysdsUtil.Czzsconvert2VO(qynb);
			Map pData = new HashMap();
	...
	protected String beforePerform(HttpServletRequest request,
		...
		request.removeAttribute(QysdsNbConstant2014.JUMP_FLAG_NAME);
		return null;
	}
	...
	public String doJump(HttpServletRequest request,
		...
		try {
			...
			request.getSession(false).setAttribute(QysdsNbConstant2014.REQ_KEY_QYSBSNB_2014, qysdsnbvo);
			//没有申报数据
