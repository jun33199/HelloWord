...
public class CzzssdsjbAction extends DcBaseAction {
	...
	protected boolean validate(CzzssdsjbVO qynb) {
		if (!qynb.getYwlx().equals(Constant.CA_YWLXDM_CZZSSDSJB_2014)) {
	...
	public String doSave(HttpServletRequest request,
		...
		try {
			...
			pData.put(QysdsksMapConstant.STRING_KEY_BBLX, Constant.TABLE_ID_CZZSSDS_2014);
			...
			vo.setData(pData);
			vo.setProcessor(ProcessorNames.CZZSQYSDSJB_PROCESSOR_2014);
			...
			if (ud.getCaflag()) {
				...
				request.getSession(false).setAttribute("REQ_SHENBAO_HUIZHI_KEY_DZYJ",hzlist);
			} else {
				request.setAttribute(CAConstants.REQ_SHENBAO_HUIZHI_KEY, "");
				request.getSession(false).setAttribute("REQ_SHENBAO_HUIZHI_KEY_DZYJ","");
			}
...
//			
			...
			pDataMap.put(QysdsksMapConstant.STRING_KEY_BBLX, Constant.TABLE_ID_CZZSSDS_2014);
			...
			//这个方法里边又从新生成了个vo...
			qysdsjbvo = qysdsUtil.Czzsconvert2XMLVO(qysdsjbbo, djJbsj, Constant.TABLE_ID_CZZSSDS_2014);
...
//			System.out.print("zfjg_old==="+nszf_old.get("zfjg_old"));
			...
			request.getSession().setAttribute(Constant.CZZSSDSJB_DM_MAPS, QysdsUtil2014.getCodeTable());
    ...
    /**
     * 跳转到固定资产加速折旧（扣除）预缴情况统计表，2014.12.05
     ...
     * @author zhangj
     */
    public String doJumpGdzc(HttpServletRequest request,
            HttpServletResponse response) throws BaseException {
        System.out.println("i am jumping...");
     ...
     * @author zhangj
     */
	public String doJumpSuccess(HttpServletRequest request,
            HttpServletResponse response) throws BaseException{
		UserData ud = (UserData) this.getUserData(request);
		request.setAttribute(CAConstants.REQ_SHENBAO_HUIZHI_KEY,request.getSession(false).getAttribute("REQ_SHENBAO_HUIZHI_KEY_DZYJ"));
		request.setAttribute(SuccessConstant.SUCCESS_MESSAGE,"企业所得税季度纳税申报表保存成功");
		request.setAttribute(CAConstants.REQ_KEY_SUCCESS_MSG,"企业所得税季度纳税申报表保存成功");
		request.setAttribute(CAConstants.REQ_KEY_RETURN_ADDRESS,Constant.REQ_KEY_RETURN_CZZSSDS_QYJB2014);
		return CAUtils.jumpTo(ud.getCaflag(),CAConstants.SAVE,CAConstants.SUCCESSSB);
	}
}
