//Source file: E:\\Generate Code\\com\\ttsoft\\bjtax\\shenbao\\sbzl\\hdzsgrsds\\web\\HdzsgrsdsAction.java
...
/**
 * 核定征收个人独资个人合伙个人所得税申报表_应税所得率
 *
 * @author Haifeng Su
 * @version 1.0
 */
public class HdzsgrsdsAction extends ShenbaoAction
{
    ...
    /**
     * 保存申报信息
     * 1、取得HdzsgrsdsForm，产生一个VOPackage。
     * 2、调用HdzsgrsdsForm的beforeSave方法取得保存申报信息的参数，
     * 	设置它为voPackage的data域。
        ...
        BaseException
    {
        ...
        //获取Form对象
        HdzsgrsdsForm hdzsgrsdsForm = (HdzsgrsdsForm)form;
        //生成VOPackage
    ...
    {
        return SbzlAccess.HDYSSDL;
    }

    /**
     * 取业务信息
     * 1、取得HdzsgrsdsForm，产生一个VOPackage。
     * 2、调用HdzsgrsdsForm的beforeQuery方法取得取业务数据的参数，
     * 	设置它为voPackage的data域。
     * 3、设置voPackage的type域为
     * 	com.ttsoft.bjtax.shenbao.constant.ProcessorNames.HdzsgrsdsProcessor，
     * 	voPackage的action域为HdzsgrsdsActionConstant.INT_ACTION_QUERY
     * 	调用ShenBaoProxy的process方法。
     * 4、调用HdzsgrsdsForm的afterQuery方法设置取业务数据的结果。
     * 5、转向业务页面。
     * @param mapping 	struts.action.ActionMapping
     * @param form	HdzsgrsdsForm
     * @param request	HttpServletRequest
     ...
     * 删除申报信息
     * 1、取得HdzsgrsdsForm，产生一个VOPackage。
     * 2、调用HdzsgrsdsForm的beforeDelete方法取得删除申报信息的参数，
     * 	设置它为voPackage的data域。
     * 3、设置voPackage的type域为
     * 	com.ttsoft.bjtax.shenbao.constant.ProcessorNames.HdzsgrsdsProcessor，
     * 	voPackage的action域为HdzsgrsdsActionConstant.INT_ACTION_DELETE调用
     ...
     * 设置代码表到form。
     * @param request HttpServletRequest
     * @param hdzsgrsdsForm HdzsgrsdsForm
     */
    private void setCodeTable(HttpServletRequest request,
                              HdzsgrsdsForm hdzsgrsdsForm)
    {
        List szsmList = CodeTableUtil.getCodeTableList(request, CodeTable.SZSM_LIST);
