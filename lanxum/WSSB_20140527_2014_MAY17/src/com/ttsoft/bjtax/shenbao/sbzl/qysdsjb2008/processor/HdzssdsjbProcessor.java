...
//		 税务登记基本数据值对象
	...
	private Map ConvertDlSbsjBoToMap(HdzssdsBO qysdsjbbo) {
		...
		map.put("2", qysdsjbbo.getYssdl());
		map.put("3", qysdsjbbo.getYnssde());
		map.put("11",qysdsjbbo.getSl());
		map.put("12", qysdsjbbo.getYnsdse());
		map.put("13", qysdsjbbo.getJmsdse());
		map.put("14", qysdsjbbo.getYyjsdse());
		map.put("15",qysdsjbbo.getYbsdse());
		return map;
	}
	...
	private void insertJm(HdzssdsBO qysdsjdbo,SWDJJBSJ djjbsj,Connection conn)throws BaseException{
		ORManager orManager = null;
		String sql=null;
		CallableStatement st = null;
