 ...
 * 
 * 项目名称：wssb_20140528   
 * 类名称：HdzssdsjbProcessor   
 * 类描述：   
 * 创建人：wangcy 
 * 创建时间：2014-6-7 下午4:27:43   
 * 修改人：wangcy   
 * 修改时间：2014-6-7 下午4:27:43   
 * 修改备注：   
 * @version  1.0
 */
public class HdzssdsjbProcessor implements Processor {
	...
	String[] HCName = { "syze", "bzssr", "mssr", "yssre", "yssdl", "ynssde",
			"sl", "ynsdse", "xwqyjmsdse","yyjsdse",
			"ybsdse","swjghdynsdse","zczb", "zcze","zgrs","sshydm"};	
	...
	private Map doSave(VOPackage vop) throws BaseException {
		...
		Connection conn = null;
		HdzssdsjbVO qyjb = (HdzssdsjbVO) data.get(CAcodeConstants.CA_MAPKEY_VO_XMLVO);
		DzyjsjVO dzyj = (DzyjsjVO) data.get(CAcodeConstants.CA_MAPKEY_VO_DZYJSJ);

		HdzssdsBO qysdsjdbo = (HdzssdsBO) data.get(QysdsksMapConstant.VO_KEY_KSSBSJ);
		// 报表类型
...
//		insertJm(qysdsjdbo,djjbsj,conn);
		...
		if (ud.getCaflag()) {
			...
			try {
				String ywid = qyjb.getNsrxx().getJsjdm()+ "+"+ DjStringUtil
...
//		PreparedStatement pstmt = null;
		...
		HdzssdsBO qysdsjbbo = (HdzssdsBO) data.get(QysdsksMapConstant.VO_KEY_KSSBSJ);
...
//			insertJm(qysdsjbbo,djjbsj,conn);
	...
	private Map ConvertDlSbsjBoToMap(HdzssdsBO qysdsjbbo) {
		...
		map.put("12",qysdsjbbo.getXwqyjmsdse());
		map.put("13", qysdsjbbo.getYyjsdse());
		map.put("14",qysdsjbbo.getYbsdse());
//		map.put("15",qysdsjbbo.getSwjghdynsdse());
		...
		map.put("16", qysdsjbbo.getZczb());
		map.put("17",qysdsjbbo.getZcze());
		map.put("18", qysdsjbbo.getZgrs());
		map.put("19",qysdsjbbo.getSshydm());
		return map;
	}
...
//				jm.setJsje(new BigDecimal(qysdsjdbo.getJmsdse()));
				...
				String jmxmdm = (new ServiceProxy()).getJmsbs(djjbsj.getJsjdm(), SzsmdmConstant.QYSDS, skssksrq,skssjsrq);
