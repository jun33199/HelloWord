...
//                        System.out.println("value = " + "0.00");
	...
	private Map ConvertSbsjBoToMap(CzzssdsBO qysdsjbbo) {
		...
		map.put("9", qysdsjbbo.getMbyqndks());
		map.put("10", qysdsjbbo.getSjlre());
		map.put("11", qysdsjbbo.getSl());
		map.put("12", qysdsjbbo.getYnsdse());
		map.put("13", qysdsjbbo.getJmsdse());
		map.put("14", qysdsjbbo.getSjyyjsdse());
		map.put("15", qysdsjbbo.getTdywyjsdse());
        map.put("16", qysdsjbbo.getYbtsdse());
        map.put("17", qysdsjbbo.getYqnddjsdse());
		map.put("18", qysdsjbbo.getBqsjybtsdse());
		map.put("24", qysdsjbbo.getZjgyftsdse());
		map.put("25", qysdsjbbo.getCzjzfpsdse());
		map.put("26", qysdsjbbo.getFzjgyftsdse());
		map.put("27", qysdsjbbo.getZjgdlscjybmyftsdse());
		map.put("28", qysdsjbbo.getZjgycxfzjgyftsdse());
		map.put("29", qysdsjbbo.getFpbl());
		map.put("30", qysdsjbbo.getFpsdse());
		return map;
	}
	...
	private void insertJm(CzzssdsBO qysdsjdbo, SWDJJBSJ djjbsj, Connection conn)
		...
		try {
			...
			if (jmxmdm != null && !("".equals(jmxmdm))) {
				...
				String ysjcdm = null;
				String yskmdm = null;
				ysjcdm = FriendHelper.getYsjc(jm.getJsjdm(), jm.getSzsmdm(),
						jm.getSkssjsrq()).getYsjcdm();
				yskmdm = JKAdapter.getInstance().getYskm(jm.getSzsmdm(),
						jm.getDjzclxdm(), jm.getGjbzhydm(), ysjcdm).getYskmdm();
