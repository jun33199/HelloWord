 ...
 * @version 1.0
 */
public class HdzsgrsdsForm extends SbzlBaseForm
{
    ...
    /**
     * 实际应缴税额	Number(15，2)	Y
     * 由应纳税额、减免税额、期初未缴税额和已缴纳税额计算公式计算出
     * 可以修改（应大于等于0）
     * String[]
...
//            {
//                BigDecimal[] temp = getLevel(ynssdeCount, this.slbsjList);
                //应纳税额＝应纳税所得额×适用税率－速算扣除数
...
//            sjyjseCount = ynsdseCount.subtract(new BigDecimal(this.
//                jmse[i])).add(new BigDecimal(this.qcwjsk[i])).subtract(new
//                BigDecimal(this.yjnse[i]));
//            if(zero.compareTo(sjyjseCount) > 0)
//            {
//                this.sjyjse[i] = "0.00";
//            } else
//            {
//                this.sjyjse[i] = sjyjseCount.toString();
//            }
...
//        }
        ...
        )
    {
        ...
        hdzstzzsb.setYnssde(new BigDecimal(ynssde));
        hdzstzzsb.setSysl(new BigDecimal(sysl));
        hdzstzzsb.setSskcs(new BigDecimal(sskcs));
