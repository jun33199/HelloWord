 ...
 */
package com.syax.bjtax.shenbao.jmba.basx05.processor;
...
public class Basx05Processor implements Processor {
	...
	private Object doSave(VOPackage vo) throws BaseException {
		...
		Jmba05VO mxvo = (Jmba05VO) vo1.getQysdsjmba().get(0);
		UserData ud = (UserData) vo.getUserData();
		...
		try {
			...
			if (mxvo.getXh() != null && !mxvo.getXh().equals("")) {
				sql = "UPDATE sfdb.sf_jl_qysdsjmsba_05 SET NLMYJMXMDM='"
						+ mxvo.getNlmyjmxmdm() + "',JMSDE='" + mxvo.getJmsde()
						+ "',LRR='" + ud.getYhid()
						+ "',LRRQ=sysdate where xh ='" + mxvo.getXh() + "'";
			} else {
				String xh = qysdsUtil.getSequence();
				sql = "INSERT INTO sfdb.sf_jl_qysdsjmsba_05 (XH,BASQWSH,JSJDM,BAND,SWJGZZJGDM"
						+ ",NLMYJMXMDM"
						+ ",JMSDE,"
	...
	private Object doDelete(VOPackage vo) throws BaseException {
		...
		try {
			 ...
			 */
			conn = DBResource.getConnection();
			String delSQL = "DELETE FROM sfdb.sf_jl_qysdsjmsba_05 t WHERE t.xh = '"
					+ bo.getXh() + "'";
	...
	private Object doQuery(VOPackage vo) throws BaseException {
		...
		try {
			...
			if (vo1.getBasqwsh() == null || "".equals(vo1.getBasqwsh())) {
				...
				sb.append("select t.xh,t.NLMYJMXMDM,t.JMSDE");

				sb
						.append(" from sfdb.sf_jl_qysdsjmsba_05 t,sfdb.sf_jl_qysdsjmsbajl c where  t.band='"
				...
				while (rs.next()) {
					System.out.println("==========NLMYJMXMDM=========="
							+ rs.getString("NLMYJMXMDM"));
					Jmba05VO mxvo = getBamxvo(rs);
					mxvo.setXh("");
			...
			sb.append(" from sfdb.sf_jl_qysdsjmsba_05 t where  basqwsh='"
					+ bo.getBasqwsh() + "'");
			...
			while (rs.next()) {
				Jmba05VO mxvo = getBamxvo(rs);
				System.out.println("=====NLMYJMXMDM====="+rs.getString("NLMYJMXMDM"));
	...
	private Jmba05VO getBamxvo(ResultSet rs) throws Exception {
		Jmba05VO mxvo = new Jmba05VO();
		// String tbblx = null;
		try {
			mxvo.setXh(rs.getString("xh"));
			mxvo.setNlmyjmxmdm(rs.getString("NLMYJMXMDM"));
			mxvo.setJmsde(MoneyUtils.format(rs.getString("JMSDE")));
