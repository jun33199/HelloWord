...
function dac_saveData(numIndex)
{
	...
	for (var i = 0;i<this.DynamicCard.Columns.length;i++)
	{
		...
		{
			case constCTDefault:
				this.DataSource[numRecordIndex][i] = dac_trimString(objTable.rows(i).cells(1).innerText);
				break;
			case constCTInput:
				this.DataSource[numRecordIndex][i] = dac_trimString(TestObject(objTable.rows(i).cells(1).firstChild,"INPUT").value);
				break;
			case constCTSelect:
			    if (this.DynamicCard.Columns[i].DataSource==null || this.DynamicCard.Columns[i].DataSource.length==null) {
			        break;
			    }
				this.DataSource[numRecordIndex][i] = TestObject(objTable.rows(i).cells(1).firstChild,"SELECT").value;
			...
			case constCTRadioButton:
				this.DataSource[numRecordIndex][i] =
						(TestObject(objTable.rows(i).cells(1).firstChild,"INPUT").checked?constTrue:constFalse);
...
function dac_loadData(numIndex)
{
	...
	for (var i = 0;i<this.DynamicCard.Columns.length;i++)
	{
		...
		{
			case constCTDefault:
				objTable.rows(i).cells(1).innerText = this.DataSource[numRecordIndex][i];//DataSource下标从零开始
				break;
			case constCTInput:
				//var ob=TestObject(objTable.rows(i).cells(1).firstChild,"INPUT");
				TestObject(objTable.rows(i).cells(1).firstChild,"INPUT").value = this.DataSource[numRecordIndex][i];
				break;
			case constCTSelect:
			    if (this.DynamicCard.Columns[i].DataSource==null || this.DynamicCard.Columns[i].DataSource.length==null) {
			        break;
			    }
			    TestObject(objTable.rows(i).cells(1).firstChild,"SELECT").value = this.DataSource[numRecordIndex][i];
				...
				break;
			case constCTRadioButton:
				TestObject(objTable.rows(i).cells(1).firstChild,"INPUT").checked =
			...
			case constCTSequence:
				//do nothing
				objTable.rows(i).cells(1).innerText = numIndex;
				break;
			default:
				alert("unknown Column Type");
				break;
...
//add by llpei
//判断页面元素是否符合要求
function TestObject(parentObject,nameOfTag){
  
	var i=0;
	while(parentObject.tagName!=nameOfTag || i>4){
	    parentObject=parentObject.firstChild;    
	    i++;
	}
	if(parentObject.tagName!=nameOfTag ){
	    alert("currrent object is not include the tag of "+nameOfTag);
	    return;
	}
	return parentObject;
