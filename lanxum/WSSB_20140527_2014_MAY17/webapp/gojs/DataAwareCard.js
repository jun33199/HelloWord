	...
	for (var i = 0;i<this.DynamicCard.Columns.length;i++)
	{
		switch(this.DynamicCard.Columns[i].Type)
		{
			case constCTDefault:
				...
				break;
			case constCTInput:
				this.DataSource[numRecordIndex][i] = dac_trimString(objTable.rows(i).cells(1).firstChild.value);
				break;
			case constCTSelect:
			    if (this.DynamicCard.Columns[i].DataSource==null || this.DynamicCard.Columns[i].DataSource.length==null) {
			        break;
			    }
				this.DataSource[numRecordIndex][i] = objTable.rows(i).cells(1).firstChild.value;
			...
			case constCTRadioButton:
				this.DataSource[numRecordIndex][i] =
						(objTable.rows(i).cells(1).firstChild.checked?constTrue:constFalse);
	...
	for (var i = 0;i<this.DynamicCard.Columns.length;i++)
	{
		switch(this.DynamicCard.Columns[i].Type)
		{
			case constCTDefault:
				...
				break;
			case constCTInput:
				objTable.rows(i).cells(1).firstChild.value = this.DataSource[numRecordIndex][i];
				break;
			case constCTSelect:
			    if (this.DynamicCard.Columns[i].DataSource==null || this.DynamicCard.Columns[i].DataSource.length==null) {
			        break;
				...
				break;
			case constCTRadioButton:
				objTable.rows(i).cells(1).firstChild.checked =
