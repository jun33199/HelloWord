 /** 检查计算机代码是否正确
  * 此方法可以同时检查出计算机代码为空的情况
  * 合法字符："_"（下划线）、数字、英文字母
  * 合法位数：7或8位
  * @return false ：不正确；true：正确
  */
  function isJsjdm(jsjdm){
    var regExp = /^[\w]{7,8}$/g;
...
//function  : 检查输入字符串是否是小数
//parameters: DigitString: String,待检查字符串的值
//			  decimalLength: int，小数的长度限制
//return    : false: 字符串中包含除数字外的字符
//			  true : else
function isDecimalString(deciamlString,decimalLength)
{
    var re = "^[0]{1}\.{1}[0]?[\\d]?[123456789]{1,}[\\d]?$";
    re = new RegExp(re,"i");
    if(re.exec(deciamlString) == null && deciamlString != null && deciamlString != "")
    {
        return false;
    }
    //有长度限制的情况
    if(decimalLength != null && deciamlString.length > decimalLength + 2)
    return false;
    return true;
}
...
/******Begin:added by yuxy(2003.05.30)****************/
//function  : 判断按键是否为数字键或者Backspace。
function isDigitKeynBackspace()
{
...
//限制字符等非数字
function permitNumber(objFormelement) {
    if (!isDigitKeynBackspace()) {objFormelement.contentEditable = false;alert('请输入数字！');}
    if (isDigitKeynBackspace()) objFormelement.contentEditable = true;
}
/******End:added by yuxy(2003.05.30)****************/
