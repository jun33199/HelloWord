...
{	
	...
	//申报信息
	applendElement("/taxdoc","sbxx",["fsdm","jd","nd","sbrq","sbrqshow","skssksrq","skssjsrq","fpblyxqq","fpblyxqz"]);
	//总机构信息
	applendElement("/taxdoc","zjgxx",["jsjdm","nsrsbh","nsrmc","srze","gzze","zcze","hj","ftse"]);
	//分支机构信息
	...
	if(fzjgxx.length > 0)
	{
		...
		for(var i = 0; i < fzjgxx.length; i++)
		{
			...
			{
				applendMxxxElement("/taxdoc/fzjgxx","mxxx",["index","nsrsbh","nsrmc","srze","gzze","zcze","hj","fpbl","fpse"],(i+1));
			}
		}
	}
	else
...
function applendMxxxElement(root,node,tags,index)
{
	...
	{
		//明细数据域ID从10~18
		getMxxxChildren(objTemp,tags[i], (i+9), index);
	}
	if (objTemp.hasChildNodes())
	{
		objNode.appendChild(objTemp);
...
//明细信息子节点实际信息
function getMxxxChildren(temp, strTag, tabIndex, rowIndex)
{
	//alert("strTag = " + strTag);
	var id = tabIndex + "." + rowIndex + "_1";
	//alert("id = " + id + "\nstrTag = " + strTag);
