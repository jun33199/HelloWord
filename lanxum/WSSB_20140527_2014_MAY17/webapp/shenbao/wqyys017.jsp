...
var jfzce = document.createElement("<input maxlength=\"15\" size=\"15\" name=\"jfzce\" value=\"0.00\" onChange=\"return(checkvalue(this,0)&&formatCurrency(this,0)&&jfzceChange(this));\">");
cell2.appendChild(jfzce);

var cell3 = row.insertCell();
cell3.className="2-td2-left";
cell3.innerHTML = "<div id=\"hssreDiv\" align=\"right\">0.00</div><input type=\"hidden\" name=\"hssre\" value=\"0.00\">";
...
function getSbsj(objForm)
{
	...
	var total=document.getElementsByName("jsje").length;
	var nodeNameArray = new Array("szsmdm","szsmmc","jsje","sl","ynse","yjnse","bqybse","jfzce","hssre");
...
function parseXml()
{
	var xslPath = "/XSLTWEB/model/010013/XSLT/"+strXSLTVersion+".xsl";
	...
	if(rootNode.getElementsByTagName("szsmList").length==0){};
...
function doSave(){
	...
	if(itemsize == 0){
		...
		itemsize = mainTable.rows.length - 3;
		if(itemsize == 1){
        	if(document.forms[0].jfzce.value == "")
			{
				alert("经费支出额不能为空！");
				document.forms[0].jfzce.focus();
				document.forms[0].jfzce.select();
				return false;
			}
			...
			{
				if(document.forms[0].jfzce[j].value == "")
				{
					alert("经费支出额不能为空！");
					document.forms[0].jfzce[j].focus();
					document.forms[0].jfzce[j].select();
					return false;
				}
...
function jfzceChange(obj)
{
	...
	for(j = 0;j < itemsize; j++)
	{
		if(document.forms[0].jfzce[j] == obj)
		{
			i = j;
			break;
		}
    ...
    if(itemsize > 1)
    {
        if(checkvalue(document.forms[0].jfzce[i],0))
		{
			jfzce = parseFloat(document.forms[0].jfzce[i].value);
        }
        var hssre = jfzce/0.85;
        document.forms[0].hssre[i].value = Math.round(hssre*100)/100;
	hssreDiv[i].innerText = document.forms[0].hssre[i].value;
	formatDIV(hssreDiv[i]);
    }
    else if(itemsize == 1)
	{
		if(checkvalue(document.forms[0].jfzce,0))
		{
			jfzce = parseFloat(document.forms[0].jfzce.value);
		}
		var hssre = jfzce/0.85;
		document.forms[0].hssre.value = Math.round(hssre*100)/100;
		hssreDiv.innerText = document.forms[0].hssre.value;
		formatDIV(hssreDiv);
    }
}
...
function jfzceOnChange(obj,par1,par2,par3){//**
	return(checkvalue(obj,par1)&&formatCurrency(obj,par2)&&jfzceChange(obj,par3));
}
</script>
...
<table width="98%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td align="center" colspan=2>
		...
		<jsp:param name="help_url" value="help/wssb/sbzl/wqyys/jfhs/wqyys-002.htm"/>
    	</jsp:include>
