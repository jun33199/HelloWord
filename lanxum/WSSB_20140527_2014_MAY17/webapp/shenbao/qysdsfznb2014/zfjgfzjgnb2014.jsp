...
{
    var urlXSL="/XSLTWEB/model/010034/XSLT/" +strXSLTVersion+".xsl";
...
function checkSaveNew()
{
	...
	var fzjgfpbl_new=document.all.fzjgfpbl[0].value;
	var fzjgfpse_new=document.all.fzjgfpse[0].value;
	var fpsehj=document.forms[0].fpsehj.value;
	var fzjgftse=document.forms[0].fzjgftse.value;	
...
//	var fpsehj=document.forms[0].fpsehj.value;
//	var fzjgftse=document.forms[0].fzjgftse.value;
//	if((parseFloat(fzjgftse).toFixed(2)-parseFloat(fpsehj).toFixed(2)) !=0){
//		alert("分支机构分摊的所得税额额必须等于分配税额合计");
//		return false; 
//	}
//
	var tempFpblHj=document.forms[0].fpblhj.value;
	var fzjgfpblHj=tempFpblHj.replace('%','');
	...
	if((parseFloat(fpsehj).toFixed(2)-parseFloat(fzjgftse).toFixed(2)) >0){
...
{
    var old  = document.forms[0].ywczlx.value;
    if((document.forms[0].ywczlx.value == 0) && confirm("确认删除企业所得税分支机构年度纳税申报表、企业所得税汇总纳税分支机构分配表?"))
    {
...
function doReturn()
{
    if(confirm(confirmReturn))
    {
		...
		document.forms["zfjgnb2012Form"].submit();
        return true;
    }
    else
    {
...
{
    return (checkvalue(obj,1) && formatCurrency(obj,0) && formate(obj));
}
...
function setFzjgInput(arrs){
	...
	setWriteData(document.all.fzjgfpbl[0], arrs[6].split(":")[1], false);
	
	checkPercent(document.all.fzjgfpbl[0]);
	
	//setReadOnlyData(document.all.fzjgfpse[0], arrs[7].split(":")[1], true);	
	setWriteData(document.all.fzjgfpse[0], arrs[7].split(":")[1], true);
	
	setInputData(document.all.nsrsbh);
	setInputData(document.all.nsrmc);
	
	 fzjgfpbl_zb=arrs[6].split(":")[1];
	 fzjgfpse_zb=arrs[7].split(":")[1];
	//新增放开分支机构分摊的所得税额 让其进行录入
	document.all.fzjgftse.readOnly=false;
	document.all.fzjgftse.className="";
}
...
function setWriteData(obj, val, format){
	//obj.value = val;
	obj.readOnly = false;
	if(format){
		formatData(obj);
...
<table width="96%" align="center" cellspacing="0" class="table-99" onKeyDown="jsjdmQuery()">
	...
	<form id="zfjgfzjgnb2014Form" name="zfjgfzjgnb2014Form" action="zfjgfzjgnb2014.dc" method="post">
