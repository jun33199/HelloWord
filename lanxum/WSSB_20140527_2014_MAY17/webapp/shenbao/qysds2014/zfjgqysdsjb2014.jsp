...
function parseXmlOnLoad()
{
	...
	//检查是否保存成功，如果保存成功则建议跳转,ADDED BY ZHANGJ,2014.12.05
	jumpGdzc();
...
function jumpGdzc(){
	...
	%>
		if(a_zfjg=="1"){//总机构跳转，分支机构不跳转
			if(confirm("请确认该企业本季度是否已享受固定资产加速折旧（扣除）政策？")){
				doSubmitXML(document.forms[0],"JumpGdzc",false,xmlDoc.xml,true);
			}else{
...
/**
* Notes: 增加行
* 2014-3-1,将appendChild更改为insertRow方式
**/
function doAddRow()
{
    //获取已有分支机构明细信息数量
    ...
    for(var i=12; i<19; i++)
    {
        ...
        switch(i)
        {
			...
			case 16:
				...
				break;
			case 17:
				NewTd.innerHTML = "<td align=left nowrap class='2-td2-left'><input type='text' name='fzjgfpbl' id=\"" + id + "\" style='text-align:right' value='0' size='13' onblur='checkPercent(this)' onchange='compute_Row_10(\"fzjgfpbl\",\"fpblhj\")'></td>";
				break;
			case 18:
	...
	{	
		...
		if(a_nsfs=="1"){
			if(a_zfjg=="1"){
				...
				//alert(Math.abs((parseFloat(fzjgftse).toFixed(2)-parseFloat(fpsehj).toFixed(2))));
				if(Math.abs((parseFloat(fzjgftse).toFixed(2)-parseFloat(fpsehj).toFixed(2))).toFixed(2) >0.01){
...
function doReturn()
{
    if(confirm(confirmReturn))
    {
        ...
        //document.forms[0].submit();
		document.forms["zfjg2014Form"].submit();
        return true;
    }
    else
    {
...
<table width="96%" align="center" cellspacing="0" class="table-99" onKeyDown="jsjdmQuery()">
	...
	<form id="zfjg2014Form" name="zfjg2014Form" action="zfjgqyjb2014.dc" method="post">
