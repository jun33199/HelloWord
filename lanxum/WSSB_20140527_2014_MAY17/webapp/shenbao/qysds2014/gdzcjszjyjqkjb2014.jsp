 ...
 */
 function formateNum(obj){
	...
	function computeZj(lc){
		...
		document.getElementById(lc+".1_1").value=ldhygdzczj==0?"":ldhygdzczj;
		numCheck(document.getElementById(lc+".1_1"));
		document.getElementById(lc+".9_1").value=yxycxkcdgdzczj==0?"":yxycxkcdgdzczj;
		numCheck(document.getElementById(lc+".9_1"));
		document.getElementById(lc+".13_1").value=zj==0?"":zj;
...
function formateNumByValue(value){
	...
	function autoCompute(){
		for(var i=2;i<=7;i++){
			...
			document.getElementById("9."+i+"_1").value=hj_yz==0?"":hj_yz;
			computeZj(9);
			document.getElementById("11."+i+"_1").value=hj_bq_jszje==0?"":hj_bq_jszje;
			computeZj(11);
			document.getElementById("13."+i+"_1").value=hj_lj_jszje==0?"":hj_lj_jszje;
			computeZj(13);
...
function checkXhgxNew(isSave){
	for(var i=2;i<=8;i++){
		for(var j=3;j<=13;j++){
			...
			if(document.getElementById(j+"."+i+"_1").value=="") continue;
			if(j==4||j==7){
				if(parseFloat(lc)<0){
					alert("该企业“行"+i+"列"+(j-2)+"("+lc+")”应≥0，请检查该项目填写数据。");
					document.getElementById(j+"."+i+"_1").focus();
					return false;
				}				
			}else{
				if (i==8&&j>=3&&j<=5) continue;
				if(parseFloat(lc)<=0){
					alert("该企业“行"+i+"列"+(j-2)+"("+lc+")”应>0，请检查该项目填写数据。");
					document.getElementById(j+"."+i+"_1").focus();
					return false;
		...
		if(k==8){
		if(document.getElementById("8."+i+"_1").value==""&& isSave==false) continue;
		if(((parseFloat(lc4)*2/3)*(parseFloat(jd)/4)).toFixed(2)<parseFloat(lc6)){
			alert("该企业“行"+i+"列6”("+lc6+")应≤“行"+i+"列4”×（2/3×100%）×季度值/4的值("+((parseFloat(lc4)*2/3)*(parseFloat(jd)/4)).toFixed(2)+")，请检查该项目填写数据。");
