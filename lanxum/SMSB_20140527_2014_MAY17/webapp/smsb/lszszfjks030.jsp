...
<html:form action="/webapp/smsb/lszsZfjksAction.do" method="POST">
 ...
 <tr>
    <td valign="top" height="300"> <br>
      <table align="center" cellspacing="0" class="table-99" height="226" width="1%">
        <tr>
          <td class="1-td1"  colspan="2" height="28" >作废零散汇总缴款书</td>
        </tr>
        <tr>
          <td class="1-td2" align="right" colspan="2" height="171" >
          <p style="text-align: right">征收机关名称：<bean:write name="lszsZfjksForm" property="swjgzzjgmc"/><html:hidden property="swjgzzjgdm" /><br>
            </p>
            <table cellspacing="0" class="table-99" width="99%" height="27">
             ...
             <b>
             <table width="100%" border="0" cellpadding="0" cellspacing="0" id="jkslist" onmouseup='dokeyUp(this.id);showDetail(this.id,"detlist")' class='black9'>
	              <tr>
	              ...
	              <!--The loop begin-->
	              <%int intCount =0; %>
                <bean:define id="jkslist" name="lszsZfjksForm" property="dataList"/>
                <logic:iterate indexId="index" name="jkslist" id="itemMap">
	              ...
	              <tr id="<ttsoft:write name="item" property="sbhzdh"/>" style="cursor:hand;">
	                ...
	                <td nowrap class="2-td2-center"><img onclick="cxhzFunc('<ttsoft:write name="item" property="jsjdm"/>','<ttsoft:write name="item" property="sbhzdh"/>','<ttsoft:write name="item" property="sbbh"/>'); return false;" style="cursor:hand" onMouseOver="MM_swapImage('ds<%=intCount%>','','<%=static_contextpath%>images/b-cx2.jpg',1)" onMouseOut="MM_swapImgRestore()" alt="作废" id="ds<%=intCount%>" src="<%=static_contextpath%>images/b-cx1.jpg" width="79" border="0" height="22"></td>
	              </tr>
	              <%intCount++;%>
                </logic:iterate>
                <!--The end of loop-->
        ...
        <tr>
          <td nowrap >
            <logic:equal value="01" name="lszsZfjksForm" property="lw">
            <table  width="100%" id="detlist" border='0' cellpadding='0' cellspacing='0' align='left' class='black9'>
              ...
              <%int detCount = 0; %>
              <bean:define id="detlist" name="lszsZfjksForm" property="detailList"/>
              ...
              <tr id="<ttsoft:write name="item2" property="sbhzdh"/>" style="display:none">
                ...
                <td nowrap class="2-td2-center" >
                  <img onclick="printSbjkdFunc('<ttsoft:write name="item2" property="sbbh"/>','<bean:write name="lszsZfjksForm" property="jsjdm" />')" style="cursor:hand" onMouseOver="MM_swapImage('ckxxsj<%=detCount%>','','<%=static_contextpath%>images/b-ckxxsj2.jpg',1)" onMouseOut="MM_swapImgRestore()" value="查看详细数据" id="ckxxsj<%=detCount%>" src="<%=static_contextpath%>images/b-ckxxsj1.jpg" width="95" height="22">
                </td>
              </tr>
              <% detCount++; %>
            </logic:iterate>
            ...
            <logic:equal value="00" name="lszsZfjksForm" property="lw">
            <table  width="100%" id="detlist" border='0' cellpadding='0' cellspacing='0' align='left' class='black9'>
              <tr>
              ...
              <tr id="<ttsoft:write name="item2" property="sbhzdh"/>" style="display:none">
                ...
                <!--<img onclick="chakanDetail('<ttsoft:write name="item2" property="ypys"/>','<ttsoft:write name="item2" property="jkpzh"/>')" style="cursor:hand" onMouseOver="MM_swapImage('ckxxsj<%=detCount%>','','<%=static_contextpath%>images/b-ckxxsj2.jpg',1)" onMouseOut="MM_swapImgRestore()" value="查看详细数据" id="ckxxsj<%=detCount%>" src="<%=static_contextpath%>images/b-ckxxsj1.jpg" width="95" height="22"></td>-->
                <td nowrap class="2-td2-center" >
                  <img onclick="printFunc('<ttsoft:write name="item2" property="jkpzh"/>','<bean:write name="lszsZfjksForm" property="jsjdm" />')" style="cursor:hand" onMouseOver="MM_swapImage('ckxxsj<%=detCount%>','','<%=static_contextpath%>images/b-ckxxsj2.jpg',1)" onMouseOut="MM_swapImgRestore()" value="查看详细数据" id="ckxxsj<%=detCount%>" src="<%=static_contextpath%>images/b-ckxxsj1.jpg" width="95" height="22">
                </td>
              </tr>
              <% detCount++; %>
            </logic:iterate>
...
function cxhzFunc(dm,hzdh,sbbh){
	if (dm!="" && hzdh!="" && sbbh!=""){
		document.lszsZfjksForm.jsjdm.value=dm;
		document.lszsZfjksForm.sbhzdh.value=hzdh;
		document.lszsZfjksForm.sbbh.value=sbbh;
		document.lszsZfjksForm.target="";
		//alert(document.lszsZfjksForm.jsjdm.value+"\n"+document.lszsZfjksForm.sbhzdh.value+"\n"+document.lszsZfjksForm.hzrq.value);
		doSubmitForm('Delete');
	}
...
function chakanDetail(bs,hm){
	if (bs!="" && hm!="") {
		document.lszsZfjksForm.jkpzh.value=hm;
		doSubmitForm(bs);
	}
}
