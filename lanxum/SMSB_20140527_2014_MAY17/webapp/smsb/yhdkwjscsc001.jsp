...
<html:form action="/webapp/smsb/yhdkwjscscAction.do" method="POST">
...
function doSave(){
	...
	if(nyValue=="") {
		alert("本月扣款文件银行已取，不可重新生成！");
		return false;
	}
	else if(yscNyValue.indexOf(nyValue) >= 0 && !window.confirm("该月的扣款信息文件已经生成过，请确认是否重新生成？")){
		return false;
	}
	if(window.confirm("即将生成该月银行扣款文件并上传，请确认")) {
