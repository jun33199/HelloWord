...
<html:form action="/webapp/smsb/lszsJkslrAction.do" method="POST">
      ...
      <table align="center" cellspacing="0" class="table-99">
        ...
        <tr>
          <td class="1-td2"  colspan="2"> <br>
            <table cellspacing="0" class="table-99" width="99%" height="27">
              <tr class="black9">
                <td align="center" nowrap  height="11">
        	        <div align="center">填发日期：<bean:write name="lszsJkslrForm" property="sbrq"/> <html:hidden property="sbrq"/>&nbsp;&nbsp;
        	        </div>
                </td>
                <td align="center" nowrap  height="11">
                	<div align="right">&nbsp;征收机关名称：<bean:write name="lszsJkslrForm" property="swjgzzjgmc"/><html:hidden property="swjgzzjgmc"/> <html:hidden property="swjgzzjgdm"/></div>
                </td>
              </tr>
            </table><br>
             ...
             <table class="table-99" border="0" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                <td nowrap class="2-td2-left"><div align="left">&nbsp;
					...
					<ttsoft:define id="ZJLX" codekey="ZJLX"/></div>
                </td>
			...
			<ttsoft:smsbtable form="lszsJkslrForm" property="dataList" tableId="SBJKMX_GTGSH" hasTitle="true"/>
			<DIV id=divSfTemp></DIV>
              ...
              <tr>
                ...
                <td width="8%"><input type="image" accesskey="w" tabIndex="-1" onclick="doSubmitForm('Dismiss');return false;" style="cursor:hand" onMouseOver="MM_swapImage('ds1','','<%=static_contextpath%>images/whjks-w2.jpg',1)" onMouseOut="MM_swapImgRestore()" alt="撤销" id="ds1" src="<%=static_contextpath%>images/whjks-w1.jpg" width="110" border="0" height="22"></td>
...
//调整缴款书录入条数4-->14 Add By TuJunbing @@2012-7-13
function befSave(){
	tableid="SBJKMX_GTGSH";
	var rows = eval(tableid).rows;
	//alert(rows.length+"<>hhhh");
...
//TSNY判断   ChangeBy Tujunbing 2012-08-30 R168-R262
...
//所属日期判断  ChangeBy Tujunbing 2012-08-30 R264-R351
//检查并比较提示日期、执行日期，需要税种税目代码、税款所属结束日期  
...
function doSubmitCheck(ope){
	...
	for(var ii=1;ii<SBJKMX_GTGSH.rows.length-1;ii++){
		var checkSzsmdm = "";
		var checkSkssjsrq = "";	
		if(SBJKMX_GTGSH.rows[ii].all("szsmdm")) {	
			var szsmdmVal = SBJKMX_GTGSH.rows[ii].all("szsmdm").value;
			if(szsmdmVal==""){
				alert("税种税目代码不可为空");	
				SBJKMX_GTGSH.rows[ii].all("szsmdm").focus();
				return false;
			}
			else{
		...
		if(compare(SBJKMX_GTGSH.rows[ii].all("skssksrq"),SBJKMX_GTGSH.rows[ii].all("skssjsrq"))<0) {
			...
			if(SBJKMX_GTGSH.rows[ii].all("skssjsrq")){
				checkSkssjsrq = SBJKMX_GTGSH.rows[ii].all("skssjsrq").value;
			}
		}
		
		//2007年城镇土地使用税新旧税率衔接
		//背景：2007-1-1日起城镇土地使用税税额调整，2007-1-1日后采用新税额，之前采用旧税额
		if(SBJKMX_GTGSH.rows[ii].all("szsmdm")&&SBJKMX_GTGSH.rows[ii].all("szsmdm").value.substr(0,2)=="15"){
			if(SBJKMX_GTGSH.rows[ii].all("skssksrq")&&SBJKMX_GTGSH.rows[ii].all("skssjsrq")){
				var ksnd=SBJKMX_GTGSH.rows[ii].all("skssksrq").value.substr(0,4);
				var jsnd=SBJKMX_GTGSH.rows[ii].all("skssjsrq").value.substr(0,4);
				if(ksnd<=2006&&jsnd>2006){
					alert("2007年1月1日起城镇土地使用税税额调整为原来的三倍，请将税款所属期拆分为：\n"+
					SBJKMX_GTGSH.rows[ii].all("skssksrq").value+"-20061231\n"+
					"20070101-"+SBJKMX_GTGSH.rows[ii].all("skssjsrq").value+"\n"+
					"分两次进行申报录入！");
					SBJKMX_GTGSH.rows[ii].all("skssjsrq").value="20061231";
					SBJKMX_GTGSH.rows[ii].all("skssjsrq").focus();
					return false;
				}
			}
		}
...
//下拉列表使用的js数组
<ttsoft:write name="lszsJkslrForm" property="scriptStr" filter="false"/>
//是否现实附加税标志
isadditions = true;
