...
<html:form action="/webapp/smsb/lszsHzjksAction.do" method="POST">
        ...
        <tr>
			...
			<table border="0" width="98%">
                    <tr class="black9">
                      <td colspan="5">&nbsp;</td>
                      <td>征收机关名称：</td>
                      <td colspan="2"><bean:write name="lszsHzjksForm" property="swjgzzjgmc"/><html:hidden property="swjgzzjgmc"/> <html:hidden property="swjgzzjgdm"/></td>
                    </tr>
                    <tr class="black9">
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
             ...
             <br>
             <logic:equal name="lszsHzjksForm" value="01" property="lw">
               <table width="100%" border="0" cellpadding="0" cellspacing="0" height="97">
               <%int intCount =0; %>
               <bean:define id="bhslist" name="lszsHzjksForm" property="bhList"/>
               ...
               <bean:define id="item" name="itemMap"/>
                 <tr>
                   ...
                   <td height="23" nowrap class="2-td2-t-center">
                      <img onclick="printFunc('<ttsoft:write name="item" property="sbbh"/>','<bean:write name="lszsHzjksForm" property="jsjdm" />')" style="cursor:hand;" onMouseOver="MM_swapImage('ckxxsj<%=intCount%>','','<%=static_contextpath%>images/b-ckxxsj2.jpg',1)" onMouseOut="MM_swapImgRestore()" alt="查看详细数据" id="ckxxsj<%=intCount%>" src="<%=static_contextpath%>images/b-ckxxsj1.jpg" width="95" height="22">
                   </td>
                   <%
                   }else{
                   %>
                 ...
                 <tr>
                   <td colspan="4" nowrap class="2-td2-center" height="23">
                     <img onclick="printFunc('<ttsoft:write name="item" property="sbbh"/>','<bean:write name="lszsHzjksForm" property="jsjdm" />')" style="cursor:hand;" onMouseOver="MM_swapImage('ckxxsj','','<%=static_contextpath%>images/b-ckxxsj2.jpg',1)" onMouseOut="MM_swapImgRestore()" alt="查看详细数据" id="ckxxsj" src="<%=static_contextpath%>images/b-ckxxsj1.jpg" width="95" height="22">
                   </td>
                 </tr>
                 -->
                 <%intCount++;%>
             ...
             <logic:notEqual name="lszsHzjksForm" value="01" property="lw">
             <table width="100%" border="0" cellpadding="0" cellspacing="0" height="97">
                <%int intZCount =0; %>
                <bean:define id="bhslist" name="lszsHzjksForm" property="bhList"/>
                <logic:iterate indexId="index" name="bhslist" id="itemMap">
                <bean:define id="item" name="itemMap"/>
                    ...
                    <td nowrap class="2-td2-center" height="18"><img onclick="printFunc('<%=flwMap.get("jkpzh")%>','<bean:write name="lszsHzjksForm" property="jsjdm" />')" style="cursor:hand" onMouseOver="MM_swapImage('ckxxsj<%=intCount%>','','<%=static_contextpath%>images/b-ckxxsj2.jpg',1)" onMouseOut="MM_swapImgRestore()" alt="查看详细数据" id="ckxxsj<%=intCount%>" src="<%=static_contextpath%>images/b-ckxxsj1.jpg" width="95" height="22"></td>
                    </tr>
                    <%
                    intCount++;
                    }
                    ...
                    <tr>
                      <!--<td nowrap class="2-td2-left" height="28">共 <bean:write name="lszsHzjksForm" property="hjzs"/> 张<html:hidden property="hjzs" size="2" /></td>-->
...
function hzQuery(hzfs){
  //alert(hzfs);
  if (isDate(document.lszsHzjksForm.hzksrq,"") && isDate(document.lszsHzjksForm.hzjsrq)){
    if (compare(document.lszsHzjksForm.hzksrq,document.lszsHzjksForm.hzjsrq)>=0){
      if(hzfs=="2"){
        //一票多税
        document.lszsHzjksForm.ypys.value=hzfs;
...
function chakanDetail(hm){
  if (hm!="") {
    document.lszsHzjksForm.jkpzh.value=hm;
    //一票一税
    if (document.lszsHzjksForm.ypys.value=="1"){
      doSubmitForm('Ypys');
    }
    //一票多税
    if (document.lszsHzjksForm.ypys.value=="2"){
      doSubmitForm('Ypds');
    }
  }
}
