...
<%@ page import="com.ttsoft.bjtax.smsb.qysdsjmsbagl.basx18.web.Basx18Form" %>
...
<html>  
  ...
  <%
  		Basx18Form basx = (Basx18Form)request.getAttribute("basx18Form");
  	// 构造减免税备案事项代码
	    if(session.getAttribute(CodeConstant.PAGE_SELECT_ITEMS_SESSION_KEY_FWYWFWDM) != null)
	    {
	        out.print("var arySelect_zrlx = new Array(");
	        out.print("[\"\",\"\"]");
	        String[][] lyzldm = (String[][])session.getAttribute(CodeConstant.PAGE_SELECT_ITEMS_SESSION_KEY_FWYWFWDM);
	...
	function doInit(){
		...
		_addOptionsToSelect(document.forms[0].zrlxSelect, arySelect_zrlx);
		document.forms[0].zrlxSelect.value = document.all("fwywfwdm").value;
	...
	function addzl(){	
		...
		if(zlmc != ""){
			...
			document.getElementById("zlmc").value = "";
			alert("录入经认定的技术先进型服务企业减免税备案事项内容不能为空");
		}
	...
	function checkYm(){
			...
			//验证"技术先进型服务业务范围"不能为空
			var index=document.all.zrlxSelect.selectedIndex;
			if(index==0 || index==-1){
				alert('"技术先进型服务业务范围"不能选择空！');
				return false;
				}
			...
			if(zsbh == ""){
				alert('"证书编号"不能为空！');
				document.all("zsbh").focus();
				return false;
			}
			
			
			//验证"证书有效期起"不能为空
			var zsqsrq=document.forms[0].zsqsrq.value;
			if(zsqsrq == ""){
				alert('"证书有效期起"不能为空！');
			...
			//验证"证书有效期止"不能为空
			var zszzrq=document.forms[0].zszzrq.value;
			if(zszzrq == ""){
				alert('"证书有效期止"不能为空！');
			...
			//验证"证书有效期起"不能晚于"证书有效期止"
			d1Arr=zsqsrq.split('-');
			d2Arr=zszzrq.split('-');
			v1=new Date(d1Arr[0],d1Arr[1],d1Arr[2]);
			v2=new Date(d2Arr[0],d2Arr[1],d2Arr[2]);
			if(v1>v2){
				alert('"证书有效期起"不能晚于"证书有效期止"');
				return false;
			}

			//验证"本年预计的减免税额"不能为空
			var bnyjjmse=document.all.bnyjjmse.value;
			if(bnyjjmse == ""){
				alert('"本年预计的减免税额"不能为空！');
				document.all.bnyjjmse.focus();
				return false;
			}

			//验证"本年预计的减免税额"的输入格式
			if(isNaN(bnyjjmse) || bnyjjmse.indexOf(".")==0 || bnyjjmse.indexOf(".")==bnyjjmse.length-1){
				alert('"本年预计的减免税额"输入格式不正确！');
				document.all.bnyjjmse.focus();
				return false;
				}
	...
	function jssq()
	{
		if(checkZbxx() && checkYm()&& checkZlRadio()){
			...
			if(window.confirm("是否接受该纳税人的减免税备案申请？")){
				//用来处理下拉框保存后页面回显
				document.forms[0].fwywfwdm.value = document.forms[0].fwywfw.value;
  ...
  <body bgcolor="#FFFFFF" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"  onload="doInit()">
    <%@ include file="../../include/header.jsp" %> 
      ...
      <html:form action="/webapp/smsb/qysds/jmsbagl/Basx18Action.do" method="post">
        <html:hidden property="actionType" />
        <html:hidden property="basqwsh" />
        <html:hidden property="basqbh" />
        <html:hidden property="jsjdm" />
        <html:hidden property="xh" />
        <html:hidden property="fwywfwdm" />
            	...
            	<table cellspacing=0 border=0 class="table-99" style="TABLE-LAYOUT:fixed">
                    <tr>
                      ...
                      <td  width="22%"class="2-td2-t-left">
                      	&nbsp;<bean:write name="basx18Form" property="basqbh"/>
                      </td>
                      <td  class="2-td2-t-left">
                          <strong>计算机代码</strong>               
                      </td>
                      <td  class="2-td2-t-left">
                      	&nbsp;<bean:write name="basx18Form" property="jsjdm"/>
                      </td>                  
                      <td  class="2-td2-t-left">                       
                          <strong>纳税人名称</strong>
                      </td>
                      <td width="22%"  class="2-td2-t-center">
                      &nbsp;<bean:write name="basx18Form" property="nsrmc"/>
                      </td>
                    </tr>
                    <tr>
                      <td  class="2-td2-left">                        
                      ...
                      <td  width="22%"class="2-td2-left">
                      	&nbsp;<bean:write name="basx18Form" property="zgsws"/>
                      </td>
                      <td  class="2-td2-left">
                          <strong>经济类型</strong>               
                      </td>
                      <td  class="2-td2-left">
                      	&nbsp;<bean:write name="basx18Form" property="jjlx"/>
                      </td>                  
                      <td  class="2-td2-left">                       
                          <strong>所属行业</strong>
                      </td>
                      <td width="22%"  class="2-td2-center">
                      	&nbsp;<bean:write name="basx18Form" property="sshy"/>
                      </td>
                    </tr>
                   <tr>
                      <td  class="2-td2-left">
                      ...
                      <td  class="2-td2-left">
                      	&nbsp;<bean:write name="basx18Form" property="lxr"/>
                      </td>                  
                      <td  class="2-td2-left">                       
                          <strong>联系电话</strong>
                      </td>
                      <td width="22%"  class="2-td2-center" colspan="3">
                       &nbsp;<bean:write name="basx18Form" property="lxdh"/>
                      </td>
                    </tr>
                  </table>
                ...
                <td class="2-td2-center" colspan="8">
            	  ...
            	  <br>
            		<table class="table-99" cellspacing="0">
                      ...
                      <html:select name="basx18Form" styleId="zrlxSelect" property="fwywfw" />
                        </div>
                      </td>
                		</tr>
                    <tr>
                      ...
                      <td class="2-td2-center">
                        <div id="mx_zsfsdm_epx_visible_1" align="left">
                          <html:text property="zsbh" maxlength="50"></html:text>
                        </div>
                      </td>
                    </tr>
                      <tr>
                      ...
                      <td class="2-td2-center">
                        <div align="left">
                         <html:text property="zsqsrq" onClick="WdatePicker()" ></html:text>
                        </div>
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td class="2-td2-center">
                        <div align="left">
                          <html:text property="zszzrq" onClick="WdatePicker()" ></html:text>
                        </div>
                      </td>
                    </tr>
          <tr>
            ...
            <td colspan="1" class="2-td2-center">
              <div id="mx_zsfsdm_epx_visible_1" align="left">
                     <html:text property="bnyjjmse" size="20" maxlength="18"></html:text>
                      元
              </div>
            </td>
          </tr>
