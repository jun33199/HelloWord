...
<html:form action="/webapp/smsb/zhsbAction.do" method="POST" >
...
<html:hidden property="sklxmc" />
      ...
      <table width="96%" align="center" cellspacing="0" class="table-99">
                <tr> 
                  <td class="1-td1"  colspan="7">综合申报</td>
                </tr>
                <tr> 
                    ...
                    <br> <table width="94%" border="0" cellpadding="0" cellspacing="0">
                      ...
                      <tr> 
                        ...
                        <td nowrap class="2-td2-center"><!-- <select name=zh>
						  ...
						  <bean:define id="bankList" name="zhsbActionForm" property="bankList" />
				...
				<ttsoft:smsbtable form="zhsbActionForm" property="dataList"  tableId="SBJKMX" hasTitle="true"/>
                      ...
                      <tr> 
                        ...
                        <td nowrap class="2-td2-left" height="28"> 　</td>
                        <td nowrap class="2-td2-left" height="28"> 　</td>
                        <td nowrap class="2-td2-center" height="28"> &nbsp<!-- <html:hidden property="xthj" /> --><html:text property="xthj" size="10"  readonly='true' styleClass="inputnoborder" /></td>
                      </tr>					  
                    </table>
                ...
                <td colspan="7"> <ul>
                    ...
                    <ul id="foldinglist" style="display:none">
                      ...
                      <ul id="foldinglist" style="display:none">
                        ...
                        <!-- <li style="list-style-image:url('<%=static_contextpath%>images/none.gif')"><a href="PG3_SBZS_005.htm">查账征收个人独资、合伙年度</a></li> -->
                        ...
                        <!-- <li style="list-style-image:url('<%=static_contextpath%>images/none.gif')"><a href="javascript:toSbzl('hdzsgrsdsAction.do?actionType=Query','q')">核定征收个人独资、合伙季（年）度(按应税所得率征收)</a></li>
                        <li style="list-style-image:url('<%=static_contextpath%>images/none.gif')"><a href="javascript:toSbzl('hdzsgrsdszslAction.do?actionType=Query'),'q'">核定征收个人独资、合伙季（年）度(按征收率征收)</a></li> -->
                      </ul>
                      ...
                      <li style="list-style-image:url('<%=static_contextpath%>images/none.gif')">10 
						...
						<!-- <li style="list-style-image:url('<%=static_contextpath%>images/none.gif')">12 
                        <a href="javascript:toSbzl('dsdzdkAction.do?actionType=Query','m')">代售代征代扣申报</a></li> -->
						<!-- <li style="list-style-image:url('<%=static_contextpath%>images/none.gif')">13 
                        <a href="javascript:toSbzl('gtgshyysrAction.do?actionType=Query')">个人所得税申报</a></li> -->
						<li id="foldheader"
							style="list-style-image:url('<%=static_contextpath%>images/zbotton-jia2.gif')">7
						企业所得税</li>
						<ul id="foldinglist" style="display:none">
							<li
								style="list-style-image:url('<%=static_contextpath%>images/none.gif')"><a
								href="javascript:toSbzl('qysdsMainAction.do?actionType=Query','y')">查帐征收企业所得税年度申报表</a></li>
							<li
								style="list-style-image:url('<%=static_contextpath%>images/none.gif')"><a
								href="javascript:toSbzl('qysdsjbAction.do?actionType=Query','q')">查帐征收企业所得税季度申报表</a></li>
							<li
								style="list-style-image:url('<%=static_contextpath%>images/none.gif')"><a
								href="javascript:toSbzl('hdzssdsnbAction.do?actionType=Query','y')">核定征收企业所得税年度申报表</a></li>
							<li
								style="list-style-image:url('<%=static_contextpath%>images/none.gif')"><a
								href="javascript:toSbzl('hdzssdsjbAction.do?actionType=Query','q')">核定征收企业所得税季度申报表</a></li>
						</ul>
                    </ul>
                  </ul></td>
              </table>
...
//是否现实附加税标示
isadditions = true;
//isadditions = <ttsoft:write name="zhsbActionForm" property="isadditons" filter="false"/>;
//是否外资企业
iswz=<ttsoft:write name="zhsbActionForm" property="wz" filter="false"/>;
var SBJKMX_list=new DTable(SBJKMX,jsArr_SBJKMX);
//SBJKMX_list.tableHead=1;
SBJKMX_list.tableTail=1;

<ttsoft:write name="zhsbActionForm" property="bankJsArray" filter="false"/>
...
var nsrzt = <ttsoft:write name="zhsbActionForm" property="nsrzt" filter="false"/>;
checkNsrzt();
//转移到申报资料使用的税款所属日期
<ttsoft:write name="zhsbActionForm" property="skssrqArr" filter="false"/>
...
function doSubmitCheck(ope){
	...
	for(var ii=0;ii<SBJKMX.rows.length;ii++){
		...
		if(SBJKMX.rows[ii].all("szsmdm")) {	
			var szsmdmVal = SBJKMX.rows[ii].all("szsmdm").value;	
			if(szsmdmVal==""){
				alert("税种税目代码不可为空");	
				SBJKMX.rows[ii].all("szsmdm").focus();
				return false;
		...
		//检查税款所属截止日期 ChangeBy Tujunbing 2012-08-30
			...
			if(SBJKMX.rows[ii].all("skssjsrq")){
				checkSkssjsrq = SBJKMX.rows[ii].all("skssjsrq").value;
			}
		}
...
<ttsoft:write name="zhsbActionForm" property="scriptStr" filter="false"/>
//重新计算附加税的计税金额和应缴金额
function resetFjs(){	
	tableid="SBJKMX";
...
//	as("setAdditions");
	...
	if(!iswz){
		...
		delSonRow(nameHead,tableid,oldCode);
	       ...
	       //修改  时间:2010/12/1   @author:guoxj
			    //是外资企业
				//内外资分类代码为1或2时只有当选择026,0277下的税种，并且该税种的税率等于5%时则不带出535070或535610，其他情况均带出附加税。.
				var list = get_subobjs("szsmlist_add",obj.value);//附加税对象	
				var addCode;
				for(var ii=0;ii<list.length;ii++){
					var addCode = list[ii][1];//附加税代码
					//修改的地方
					if((szsmdm.substr(0,3)=='026' || szsmdm.substr(0,4)=='0277') && sl==0.05){
