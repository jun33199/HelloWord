...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.jbxxb.web;
...
public class JbxxbAction extends SmsbAction {
	...
	public ActionForward doShow(ActionMapping mapping, ActionForm form,
		...
		//获取基本信息
		JbxxbForm jbxxbForm = (JbxxbForm) form;
		jbxxbForm.reset(mapping, request);
		userData = this.getUserData(request);	
		VOPackage vo = new VOPackage();
		vo.setAction(CodeConstant.SMSB_SHOWACTION);
		vo.setData(jbxxbForm);
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_JBXXB_PROCESSOR);
		vo.setUserData(userData);
		try {
			//调用process
			jbxxbForm = (JbxxbForm) SbzlProxy.getInstance().process(vo);
...
/*//    	-------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
	 ...
	 * @return
	 */
	private CheckBean setCheckInf(JbxxbForm jbxxbForm)
	{
