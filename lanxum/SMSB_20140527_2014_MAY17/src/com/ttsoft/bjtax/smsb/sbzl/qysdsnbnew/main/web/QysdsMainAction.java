package com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.main.web;
...
public class QysdsMainAction extends SmsbAction {
	/**
	 * 操作员数据
	 */
	...
	throws BaseException {
		...
		vo.setData(baseForm);
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_MAIN_PROCESSOR);
	...
	throws BaseException {
		...
		try {
			...
			if(baseForm.getCheckList()==null){
				...
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(baseForm.getCheckList()));
			}
		} catch (Exception ex) {
			baseForm.reset(mapping, request);
			// 系统捕捉异常，根据异常类型抛出
	...
	throws BaseException {
		...
		try {
			...
			if(sessionForm!=null){
				sessionForm.setLinkUrlHTML(QysdsNewUtil.getLinkUrlHtml(sessionForm.getTableList(),sessionForm));
				request.setAttribute(mapping.getAttribute(), sessionForm);
			}else{
				baseForm = (QysdsNewForm) SbzlProxy.getInstance().process(vo);
				baseForm.setLinkUrlHTML(QysdsNewUtil.getLinkUrlHtml(baseForm.getTableList(),baseForm));
				request.setAttribute(mapping.getAttribute(), baseForm);
			}
