...
import com.ttsoft.bjtax.smsb.sbzl.grsds2014.web.GrsdsJbxxActionForm;
...
public class GrsdsJbxxbProcessor implements Processor{
	...
	private Object doQuery(VOPackage vo) throws Exception	{
		...
		try {
			...
			//如果当前有信息则取当年信息
			if(getYearQY(grsdsJbxxActionForm,Integer.parseInt(grsdsJbxxActionForm.getNd()) ,conn)){
				if(!getYearTzz(grsdsJbxxActionForm ,Integer.parseInt(grsdsJbxxActionForm.getNd()),conn)){
					getGrDefaultInf(grsdsJbxxActionForm ,conn);
				}
				
			//否则取前一年信息
			}else if(getYearQY(grsdsJbxxActionForm ,Integer.parseInt(grsdsJbxxActionForm.getNd())-1 ,conn)){
				if(!getYearTzz(grsdsJbxxActionForm ,Integer.parseInt(grsdsJbxxActionForm.getNd())-1 ,conn)){
					getGrDefaultInf(grsdsJbxxActionForm ,conn);
				}
			...
			//如果还没有则取系统信息
				...
				//取得其他默认基本数据
				getQyDefaultInf(grsdsJbxxActionForm ,conn);
				getGrDefaultInf(grsdsJbxxActionForm ,conn);
			}
	 ...
	 * @throws Exception 
	 */
	private void getQyDefaultInf(GrsdsJbxxActionForm jcxxb ,Connection conn) throws Exception{
	...
	private void getGrDefaultInf(GrsdsJbxxActionForm jcxxb ,Connection conn) throws Exception{
	 ...
	 * @throws Exception 
	 */
	private boolean getYearTzz(GrsdsJbxxActionForm jbxxb ,int nd ,Connection conn) throws Exception
	{
