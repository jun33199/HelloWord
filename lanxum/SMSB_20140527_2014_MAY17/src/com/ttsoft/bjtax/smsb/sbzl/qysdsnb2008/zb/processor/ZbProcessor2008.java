...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.zb.processor;
...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.zb.web.ZbForm2008;
...
public class ZbProcessor2008 implements Processor {
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		ZbForm2008 zbForm = (ZbForm2008) vo.getData();
		...
		try {
			...
			QysdsUtil2008.setQysdsReport(report, zbForm);
			// 设置QysdsReportsTableDeclare的基本信息
			QysdsReportsTableDeclare table = new QysdsReportsTableDeclare();
			table.setTableId(CodeConstant.TABLE_ID_2008_ZB);
			table.setTableName(CodeConstant.TABLE_NAME_2008_ZB);
			...
			table = (QysdsReportsTableDeclare) report.getTableContentList()
					.get(CodeConstant.TABLE_ID_2008_ZB);
			// 将数据库中的数据翻译成页面所需数据格式
			int[] arrs = { 1, 42 };
			zbForm.setDataList(this.translate2Page(QysdsUtil2008.putSpace(table,
					arrs)));
		} catch (Exception ex) {
			// 抛出异常
			ex.printStackTrace();
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(ZbForm2008 form) {
	...
	private List translate2Page(QysdsReportsTableDeclare table) {
		...
		Debug.out("----start---2008版 企业所得税 年度申报主表----translate2Page");
		while (it.hasNext()) {
			QysdsReportsItemDeclare item = new QysdsReportsItemDeclare();
		...
		Debug.out("----over---2008版 企业所得税 年度申报主表----translate2Page");
		return pagelist;
	}
	...
	private void saveJM(VOPackage vo) throws BaseException {
		...
		ZbForm2008 form = (ZbForm2008) vo.getData();
...
//				ts_lrrq, ts_sbrq, ts_Skssjsrq, ts_Skssksrq, time);
	 ...
	 *            当前时间
	 */
	private void putJm(String jmje, QysdsSet sdsInfo, ZbForm2008 form,
