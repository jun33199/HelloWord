...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.mbksmxb.processor;
...
public class MbksmxbProcessor
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		MbksmxbForm nbForm = (MbksmxbForm) vo.getData();
...
//			将ActionForm中的数据结构转换置入数据库接口参数要求的数据结构
			...
			table.setTableId(CodeConstant.TABLE_ID_MBKSMXB);
			table.setTableName(CodeConstant.TABLE_NAME_MBKSMXB);
...
//			 获取数据库接口，调用save方法进行数据保存
			...
			table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_MBKSMXB);
...
//			抛出异常
	...
	private Object doSave(VOPackage vo) throws BaseException
	{
		...
		MbksmxbForm mbksmxbForm = (MbksmxbForm)vo.getData();
		Connection con = null;
		try
		{
			//获取数据库连接
...
//			获取数据库接口，调用save方法进行数据保存
	...
	{
		MbksmxbForm form = (MbksmxbForm)vo.getData();
		Connection conn = null;
		try
		{
			//获取数据库连接
...
//			获取数据库接口，调用save方法进行数据保存
			...
			try
			{
				...
				table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_MBKSMXB);
				int[] arrs={1,10,11,20,21,29,30,37,38,43,44,49,50,50};
				form.setDataList(this.translate2Page(QysdsNewUtil.putSpace(table,arrs)));
			}catch(Exception ex)
			{ 
				 ex.printStackTrace();
			}
...
//			释放数据库连接
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(MbksmxbForm nbForm)
	{
//		企业所得税报表申明对象
		QysdsReportsDeclare report =new QysdsReportsDeclare();
