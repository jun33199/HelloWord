...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.cbmxbybqy.web.CbmxbybqyForm;
 ...
 * @version 1.1
 */
public class CbmxbybqyAction extends SmsbAction {
	...
	public ActionForward doShow(ActionMapping mapping, ActionForm form,
			...
			throws BaseException {
		// 获取CbmxbybqyForm对象
		CbmxbybqyForm cbmxbybqyForm = (CbmxbybqyForm) form;
		// 获取本缴费系统用户信息
		...
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_CBMXBYBQY_PROCESSOR);
		// 设置本缴费系统用户信息
		vo.setUserData(userData);
		try {
			// 调用Proxy，执行processor,获取cbmxbybqyForm返回值
			cbmxbybqyForm = (CbmxbybqyForm) SbzlProxy.getInstance().process(vo);
	...
	public ActionForward doCheck(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if(cbmxbybqyForm.getCheckList()==null){
				...
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(cbmxbybqyForm.getCheckList()));
			}
	...
	/**
	 * 从session中获取基本信息，并放入CbmxbybqyForm对象中
	 * 
	 * @param request
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request, CbmxbybqyForm form) {
		// 获取本缴费系统用户信息
		userData = this.getUserData(request);
		...
		if (baseForm != null) {
			...
			form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_CBMXBYBQY));
			form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_CBMXBYBQY));
		}
	}
}