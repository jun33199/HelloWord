...
public class NstzmxjsbAction extends 
   SmsbAction 
   {
	/**
     * 操作员数据
    ...
    public ActionForward doShow (ActionMapping mapping,
        ...
        BaseException
    {
        NstzmxjsbForm nstzmxjsbForm = (NstzmxjsbForm)form;
        this.getBaseForm(request, nstzmxjsbForm); //加入纳税人基本信息
        userData = this.getUserData(request);
        
        VOPackage vo = new VOPackage();
        vo.setAction(CodeConstant.SMSB_SHOWACTION); //设置操作类型
    	vo.setData(nstzmxjsbForm); //设置操作数据
    	vo.setUserData(userData);
    	vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_NSTZMXJSB_PROCESSOR); //设置PROCESSOR
    	try
    	{
    		nstzmxjsbForm=(NstzmxjsbForm)SbzlProxy.getInstance().process(vo);
    		request.setAttribute(mapping.getAttribute(), nstzmxjsbForm); //查询结果放入REQUEST
    	}catch(Exception ex)
    	{
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
    	...
    	NstzmxjsbForm nstzmxjsbform = (NstzmxjsbForm)form;
    	userData = this.getUserData(request);
    	String[] columns = nstzmxjsbform.getSb_cloumns();
    	String[] nstzjs_columns = nstzmxjsbform.getNstzjs_columns();

    	nstzmxjsbform.setDataList(SfRequestUtil.getValuesToList(request, columns)); //固定行信息放入FORM
    	nstzmxjsbform.setNstzjs_List(this.getValuesToList(request, nstzjs_columns)); //动态行数据放入FORM
    	this.getBaseForm(request, nstzmxjsbform);  //加入纳税人基本数据
    	request.setAttribute(mapping.getAttribute(), nstzmxjsbform);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
    	...
    	NstzmxjsbForm nbform = (NstzmxjsbForm)form;
	userData = this.getUserData(request);
	this.getBaseForm(request, nbform); //加入纳税人基本信息
	
	VOPackage vo = new VOPackage();
	vo.setAction(CodeConstant.SMSB_DELETEACTION); //设置操作类型
	vo.setUserData(userData);
	vo.setData(nbform); //设置操作数据
	vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_NSTZMXJSB_PROCESSOR); //设置PROCESSOR
	try
	{
//		调用processor
		nbform=(NstzmxjsbForm)SbzlProxy.getInstance().process(vo);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
    	...
    	String[] columns = nstzmxjsbform.getSb_cloumns();
    	String[] nstzjs_columns = nstzmxjsbform.getNstzjs_columns();    	
    	nstzmxjsbform.setDataList(SfRequestUtil.getValuesToList(request, columns)); //设置固定行数据
    	nstzmxjsbform.setNstzjs_List(this.getValuesToList(request, nstzjs_columns)); //设置动态行数据
    	this.getBaseForm(request, nstzmxjsbform); //加入纳税人基本信息
    	request.setAttribute(mapping.getAttribute(), nstzmxjsbform);
...
//			调用processor
			nstzmxjsbform=(NstzmxjsbForm)SbzlProxy.getInstance().process(vo);
			  request.setAttribute(mapping.getAttribute(), nstzmxjsbform);
				...
				if(nstzmxjsbform.getCheckList()==null){
					request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
				}else if(nstzmxjsbform.getCheckList().size()==0){
					request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
				}else if(nstzmxjsbform.getCheckList().size()>0){
					request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(nstzmxjsbform.getCheckList()));
				}
				return mapping.findForward("Show");
		}catch(Exception ex)
		{
//			系统捕捉异常，根据异常类型抛出
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,NstzmxjsbForm form)
	{
		...
		if(baseForm!=null)
		{
			...
			form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_NSTZMXJSB));
			form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_NSTZMXJSB));
		}
	}
