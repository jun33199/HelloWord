 ...
 * </p>
 * <p>
 * Description:2014企业所得税分支机构年度纳税申报表
 * </p>
 *
 * @author wangcy 
 * @version 1.2
...
public class CzzsfzjgNbProcessor implements Processor {
	...
	public Object process(VOPackage vo) throws BaseException {
		...
		//初始化FORM的标志位
		CzzsfzjgNbForm form = (CzzsfzjgNbForm) vo.getData();
		form.setSAVE_FLAG("0");
	...
	{
		// 得到Action传递过来CzzsfzjgNbForm对象
		CzzsfzjgNbForm czzsfzjgNbForm = (CzzsfzjgNbForm) vo.getData();
		...
		try {
			...
			System.out.println(czzsfzjgNbForm.getJsjdm()+ "czzsfzjgNbForm.getSbrq()：" + czzsfzjgNbForm.getSbrq());
			System.out.println(czzsfzjgNbForm.getJsjdm()+ "czzsfzjgNbForm.getSkssjsrq()：" + czzsfzjgNbForm.getSkssjsrq());

			String nd = czzsfzjgNbForm.getSkssksrq().substring(0, 4);
			// 设置期号，年报期号为1
			czzsfzjgNbForm.setQh("1");
			// 设置年度
			czzsfzjgNbForm.setSknd(nd);

			// 设置form中其它所需属性
			czzsfzjgNbForm = (CzzsfzjgNbForm) QysdsCzzsNb2014Util.queryDjxxByInterfaceDJ(conn, czzsfzjgNbForm, vo.getUserData());

			System.out.println("企业征收类型1_" + czzsfzjgNbForm.getQyzslx());
			// 税费核定信息
			this.getHdxx(czzsfzjgNbForm);
			System.out.println("企业征收类型2_" + czzsfzjgNbForm.getQyzslx());
			/* 征收方式 */
			String zsfs = czzsfzjgNbForm.getZsfs();

			System.out.println(czzsfzjgNbForm.getJsjdm() + "的征收方式代码：" + zsfs);
			...
			QysdsNewUtil.setQysdsReport(report, czzsfzjgNbForm);
			report.setVersion(QysdsConstant2014.SBZL_QYSDSNB_VERSION_2014);
			...
			if (table.getCellContentList().size() > 0) {
				czzsfzjgNbForm.setQueryFlag("1");
				czzsfzjgNbForm.setSbrq(TinyTools.Date2String(report.getSbrq(),"yyyyMMdd"));
				czzsfzjgNbForm.setSkssksrq(TinyTools.Date2String(report.getSkssksrq(), "yyyyMMdd"));
				czzsfzjgNbForm.setSkssjsrq(TinyTools.Date2String(report.getSkssjsrq(), "yyyyMMdd"));
			}else{
				czzsfzjgNbForm.setQueryFlag("0");
			}

			// 将数据库中的数据翻译成页面所需数据格式
			int[] arrs = { 1, 37 };
			czzsfzjgNbForm.setQysdsnbList(this.translate2Page(putSpace(table,arrs), zsfs));
	 ...
	 * @throws BaseException
	 */
	private void getHdxx(CzzsfzjgNbForm form) throws BaseException {
	...
	private List translate2Page(QysdsReportsTableDeclare table, String zsfs) {
		...
		if (table.getCellContentList().get("36") != null) {
			map36.put("hc", "36");
			map36.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("36")).getItemValue());
			pagelist.add(map36);
		}
		if (table.getCellContentList().get("37") != null) {
			map37.put("hc", "37");
			map37.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("37")).getItemValue());
			pagelist.add(map37);
		}
		// 返回List对象
		return pagelist;
	...
	private Object doSave(VOPackage vo) throws BaseException {
		...
		try {
			...
			QysdsReportsDeclare report = this.translate2Interface(czzsfzjgNbForm, zsfs);
			// 获取数据库应用接口
			IAppAccess iApp = AppAccessFactory.getAInstance(conn,AppAccessFactory.ACCESS_MODEL_APP_QYSDS);
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (i == 12) {
					...
					System.out.println(czzsfzjgNbForm.getJsjdm() + "的减免行次:"+ (String) map.get("hc"));
					System.out.println(czzsfzjgNbForm.getJsjdm() + "的减免行次:"+ (String) map.get("lje"));
					...
					try {
						djxx = InterfaceDj.getJBSJ_New(czzsfzjgNbForm.getJsjdm(), ud);
					} catch (Exception ex1) {
						throw ExceptionUtil.getBaseException(ex1);
					}
					Timestamp skssksrq = QysdsNewUtil.getTimestamp(czzsfzjgNbForm.getSkssksrq());
					Timestamp skssjsrq = QysdsNewUtil.getTimestamp(czzsfzjgNbForm.getSkssjsrq());
					Timestamp sbrq = QysdsNewUtil.getTimestamp(czzsfzjgNbForm.getSbrq());

					Timestamp now = new Timestamp(System.currentTimeMillis());

					// 将一般减免插入减免申报方法
					this.insertJmJb(((String) map.get("lje")),czzsfzjgNbForm, da, ud, djxx, now, now, sbrq, skssjsrq,skssksrq, nd);
			...
			czzsfzjgNbForm = (CzzsfzjgNbForm) this.doShow(vo);
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(CzzsfzjgNbForm form,String zsfs) throws ApplicationException {
	 ...
	 *            年度
	 */
	private void insertJmJb(String jmje, CzzsfzjgNbForm form,
	...
	private Object doDelete(VOPackage vo) throws BaseException {
		...
		try {
			...
			this.insertJmJb(jmje, czzsfzjgNbForm, da, ud, djxx, now, now, sbrq,skssjsrq, skssksrq, nd);
			...
			// 将数据库中的数据翻译成页面所需数据格式
			int[] arrs = { 1, 37 };
			czzsfzjgNbForm.setQysdsnbList(null);
