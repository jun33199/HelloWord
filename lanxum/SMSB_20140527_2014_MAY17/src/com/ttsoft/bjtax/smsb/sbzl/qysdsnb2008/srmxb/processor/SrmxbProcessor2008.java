...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.srmxb.processor;
...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.srmxb.web.SrmxbForm2008;
...
public class SrmxbProcessor2008
implements Processor 
{
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		SrmxbForm2008 nbForm = (SrmxbForm2008) vo.getData();
...
//			将ActionForm中的数据结构转换置入数据库接口参数要求的数据结构
			...
			table.setTableName(CodeConstant.TABLE_NAME_2008_1_1);
			table.setTbblx(Constants.CREPORTS_IBBQLX_TYPE_YEAR);
			report.getTableContentList().put(table.getTableId(), table);
//			获取数据库接口，调用save方法进行数据保存
			IAppAccess iApp = AppAccessFactory.getAInstance(conn,AppAccessFactory.ACCESS_MODEL_APP_QYSDS);
			iApp.querySingleTable(report);
			table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2008_1_1);
			int[] arrs={1,26};
			nbForm.setSrmxb(this.translate2Page( QysdsUtil2008.putSpace(table,arrs)));
...
//			抛出异常
	...
	{
		SrmxbForm2008 srmxbForm = (SrmxbForm2008)vo.getData();
		Connection con = null;
		try
		{
			//获取数据库连接
...
//			更新审核状态为“保存成功”
	...
	{
		SrmxbForm2008 form = (SrmxbForm2008)vo.getData();
		Connection conn = null;
		try
		{
//			获取数据库连接
...
//			获取数据库接口，调用save方法进行数据保存
			...
			try
			{
				...
				table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2008_1_1);
				int[] arrs={1,26};
				form.setSrmxb(this.translate2Page( QysdsUtil2008.putSpace(table,arrs)));
			}catch(Exception ex)
			{
				ex.printStackTrace();
			}
...
//			将ActionForm中的数据结构转换置入数据库接口参数要求的数据结构
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(SrmxbForm2008 nbForm)
	{
//		企业所得税报表申明对象
		QysdsReportsDeclare report =new QysdsReportsDeclare();
