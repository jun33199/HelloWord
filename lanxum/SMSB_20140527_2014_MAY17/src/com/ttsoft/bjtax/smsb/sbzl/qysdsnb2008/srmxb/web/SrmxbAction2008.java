...
public class SrmxbAction2008 extends 
   SmsbAction 
   {
        ...
        BaseException
    {
		...
		vo.setProcessor(CodeConstant.SBZL_QYSDSNB2008_SRMXB_PROCESSOR2008); //设置PROCESSOR
		try
		{
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
        ...
        userData = this.getUserData(request);
   	  ...
   	  vo.setData(srmxbForm); //设置操作数据
   	  vo.setUserData(userData);
   	  vo.setProcessor("com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.srmxb.processor.SrmxbProcessor2008"); //设置PROCESSOR
   	  try
   	  {
   		srmxbForm = (SrmxbForm2008)SbzlProxy.getInstance().process(vo);
   		request.setAttribute(mapping.getAttribute(), srmxbForm);
