...
public class QysdsMainAction2008 extends SmsbAction {
	/**
	 * 操作员数据
	 */
	...
	throws BaseException {
		...
		vo.setData(baseForm);
		vo.setProcessor("com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.main.processor.QysdsMainProcessor2008");
	...
	throws BaseException {
		...
		try {
			...
			if(baseForm.getCheckList()==null){
				...
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsUtil2008.getCheckResults(baseForm.getCheckList()));
			}
		} catch (Exception ex) {
			baseForm.reset(mapping, request);
			// 系统捕捉异常，根据异常类型抛出
	...
	throws BaseException {
		...
		try {
			...
			if(sessionForm!=null){
				sessionForm.setLinkUrlHTML(QysdsUtil2008.getLinkUrlHtml(sessionForm.getTableList(),sessionForm));
				request.setAttribute(mapping.getAttribute(), sessionForm);
			}else{
				baseForm = (QysdsNewForm) SbzlProxy.getInstance().process(vo);
				baseForm.setLinkUrlHTML(QysdsUtil2008.getLinkUrlHtml(baseForm.getTableList(),baseForm));
				request.setAttribute(mapping.getAttribute(), baseForm);
			}
