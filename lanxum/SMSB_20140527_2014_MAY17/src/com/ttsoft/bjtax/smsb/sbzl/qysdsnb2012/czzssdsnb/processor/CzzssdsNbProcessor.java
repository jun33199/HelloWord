 ...
 *
 */
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2012.czzssdsnb.processor;
 ...
 * </p>
 * <p>
 * Description:2012企业所得税分支机构年度纳税申报表
 * </p>
 *
 * @author wangcy 
 * @version 1.2
 */
public class CzzssdsNbProcessor implements Processor {

	// 企业所得税税率
	private static final String QYSDS_SL = "0.25";

    private QysdsNb2012Util util = new QysdsNb2012Util();
	...
	public Object process(VOPackage vo) throws BaseException {
		...
		//初始化FORM的标志位
		CzzssdsNbForm form = (CzzssdsNbForm) vo.getData();
		form.setSAVE_FLAG("0");
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		Map skssrq = Skssrq.yearSkssrq(curDate);
	...
	{
		// 得到Action传递过来CzzssdsNbForm对象
		CzzssdsNbForm czzssdsNbForm = (CzzssdsNbForm) vo.getData();
		...
		try {
			...
			System.out.println(czzssdsNbForm.getJsjdm()+ "czzssdsNbForm.getSbrq()：" + czzssdsNbForm.getSbrq());
			System.out.println(czzssdsNbForm.getJsjdm()+ "czzssdsNbForm.getSkssjsrq()：" + czzssdsNbForm.getSkssjsrq());

			String nd = czzssdsNbForm.getSkssksrq().substring(0, 4);
			// 设置期号，年报期号为1
			czzssdsNbForm.setQh("1");
			// 设置年度
			czzssdsNbForm.setSknd(nd);

			// 设置form中其它所需属性
			czzssdsNbForm = (CzzssdsNbForm) QysdsNb2012Util.queryDjxxByInterfaceDJ(conn, czzssdsNbForm, vo.getUserData());

			System.out.println("企业征收类型1_" + czzssdsNbForm.getQyzslx());
			// 税费核定信息
			this.getHdxx(czzssdsNbForm);
			System.out.println("企业征收类型2_" + czzssdsNbForm.getQyzslx());
			/* 征收方式 */
			String zsfs = czzssdsNbForm.getZsfs();

			System.out.println(czzssdsNbForm.getJsjdm() + "的征收方式代码：" + zsfs);
			...
			if (!(CodeConstant.ZSFSDM_CZZS.equals(zsfs))) {
				throw new ApplicationException(
						"该企业已认定为核定征收户，不能在此录入，请录入相应年度申报表！");
			}

			// 创建QysdsReportsDeclare对象
			QysdsReportsDeclare report = new QysdsReportsDeclare();
			// 将form中的基本信息转入QysdsReportsDeclare report 中
			QysdsNewUtil.setQysdsReport(report, czzssdsNbForm);
			...
			table.setTableName(CodeConstant.TABLE_NAME_CZZSSDSNB_2012);
			...
			// 获取查询到的具体数据
			table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_CZZSSDSNB_2012);

			if (table.getCellContentList().size() > 0) {
				czzssdsNbForm.setQueryFlag("1");
				czzssdsNbForm.setSbrq(TinyTools.Date2String(report.getSbrq(),"yyyyMMdd"));
				czzssdsNbForm.setSkssksrq(TinyTools.Date2String(report.getSkssksrq(), "yyyyMMdd"));
				czzssdsNbForm.setSkssjsrq(TinyTools.Date2String(report.getSkssjsrq(), "yyyyMMdd"));
			}else{
				czzssdsNbForm.setQueryFlag("0");
			}

			// 将数据库中的数据翻译成页面所需数据格式
			int[] arrs = { 1, 32 };
			czzssdsNbForm.setQysdsnbList(this.translate2Page(putSpace(table,arrs), zsfs));
	 ...
	 * @throws BaseException
	 */
	private void getHdxx(CzzssdsNbForm form) throws BaseException {
		...
		// 从申报页面取得申报日期和税款所属日期
		Timestamp sbrq = QysdsNb2012Util.getNxetTimestamp(form.getSkssjsrq());

		// Map getsbjd = this.quarterSkssrq(sbrq);
		Timestamp skssksrq = QysdsNb2012Util.getTimestamp(form.getSkssksrq());
		Timestamp skssjsrq = QysdsNb2012Util.getTimestamp(form.getSkssjsrq());
		...
		if (!(form.getXzqy() != null && form.getXzqy().equals("1")) && qysdsSet.getYbjmsl() != null) {
			// 非乡镇企业的减免情况
			form.setJmzg("1"); // 有减免资格
	...
	private List translate2Page(QysdsReportsTableDeclare table, String zsfs) {
		...
		if (table.getCellContentList().get("24") != null) {
			map24.put("hc", "24");
			map24.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("24")).getItemValue());
			pagelist.add(map24);
		}
		if (table.getCellContentList().get("25") != null) {
			map25.put("hc", "25");
			map25.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("25")).getItemValue());
			pagelist.add(map25);
		}
		if (table.getCellContentList().get("26") != null) {
			map26.put("hc", "26");
			map26.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("26")).getItemValue());
			pagelist.add(map26);
		}
		if (table.getCellContentList().get("27") != null) {
			map27.put("hc", "27");
			map27.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("27")).getItemValue());
			pagelist.add(map27);
		}
		if (table.getCellContentList().get("28") != null) {
			map28.put("hc", "28");
			map28.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("28")).getItemValue());
			pagelist.add(map28);
		}
		if (table.getCellContentList().get("29") != null) {
			map29.put("hc", "29");
			map29.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("29")).getItemValue());
			pagelist.add(map29);
		}
		if (table.getCellContentList().get("30") != null) {
			map30.put("hc", "30");
			map30.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("30")).getItemValue());
			pagelist.add(map30);
		}
		if (table.getCellContentList().get("31") != null) {
			map31.put("hc", "31");
			map31.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("31")).getItemValue());
			pagelist.add(map31);
		}
		if (table.getCellContentList().get("32") != null) {
			map32.put("hc", "32");
			map32.put("lje", ((QysdsReportsItemDeclare) table.getCellContentList().get("32")).getItemValue());
			pagelist.add(map32);
		}
		// 返回List对象
		return pagelist;
	...
	private Object doSave(VOPackage vo) throws BaseException {
		...
		// 获取税款所属季度 因为是年报 所以期号为1
		String  qh= "1";
		String nd = czzssdsNbForm.getSkssksrq().substring(0, 4);
		// 设置季度
		czzssdsNbForm.setQh(qh);
		// 设置年度
		czzssdsNbForm.setSknd(nd);
		...
		try {
			...
			QysdsReportsDeclare report = this.translate2Interface(czzssdsNbForm, zsfs);
			// 获取数据库应用接口
			IAppAccess iApp = AppAccessFactory.getAInstance(conn,AppAccessFactory.ACCESS_MODEL_APP_QYSDS);
			 ...
			 * 保存减免税
			 */
			List list = czzssdsNbForm.getQysdsnbList();
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (i == 12) {
					...
					System.out.println(czzssdsNbForm.getJsjdm() + "的减免行次:"+ (String) map.get("hc"));
					System.out.println(czzssdsNbForm.getJsjdm() + "的减免行次:"+ (String) map.get("lje"));
					...
					try {
						djxx = InterfaceDj.getJBSJ_New(czzssdsNbForm.getJsjdm(), ud);
					} catch (Exception ex1) {
						throw ExceptionUtil.getBaseException(ex1);
					}
					Timestamp skssksrq = QysdsNewUtil.getTimestamp(czzssdsNbForm.getSkssksrq());
					Timestamp skssjsrq = QysdsNewUtil.getTimestamp(czzssdsNbForm.getSkssjsrq());
					Timestamp sbrq = QysdsNewUtil.getTimestamp(czzssdsNbForm.getSbrq());

					Timestamp now = new Timestamp(System.currentTimeMillis());

					// 将一般减免插入减免申报方法
					this.insertJmJb(((String) map.get("lje")),czzssdsNbForm, da, ud, djxx, now, now, sbrq, skssjsrq,skssksrq, nd);

				}
			}

			// 获取一个具有空值的QysdsReportsTableDeclare对象
			QysdsReportsTableDeclare table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_CZZSSDSNB_2012);
			...
			czzssdsNbForm = (CzzssdsNbForm) this.doShow(vo);
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(CzzssdsNbForm form,String zsfs) throws ApplicationException {
	 ...
	 *            年度
	 */
	private void insertJmJb(String jmje, CzzssdsNbForm form,
		...
		try {
			...
			// 减免类别代码
			ServiceProxy proxy = new ServiceProxy();
			String jmxmdm = proxy.getJmsbs(form.getJsjdm(),CodeConstant.SZSM_QYSDS, ts_Skssksrq, ts_Skssjsrq);
			Debug.out("减免类别代码 =" + jmxmdm);
			System.out.println("减免类别代码 =" + jmxmdm);
			// 减免项目代码为空，从表单中取得企业的征收类型判断减免方式
			if (jmxmdm == null) {

				System.out.println("**************   取得的减免项目代码为空，上门申报的季报表中putJm中的企业征收类型form.getQyzslx()为:"+ form.getQyzslx());
			...
			if (jmxmdm != null && !("".equals(jmxmdm))) {
				...
				Ysjc ysjc = null;
				try {
					ysjc = JksUtil.getYsjc(form.getJsjdm(),CodeConstant.SZSM_QYSDS, SfDateUtil.getDate(form.getSbrq()));
				} catch (Exception e) {
				...
				try {
					yskm = JKAdapter.getInstance().getYskm(CodeConstant.SZSM_QYSDSCODE, djsj.getDjzclxdm(),djsj.getGjbzhydm(), ysjc.getYsjcdm());
				} catch (Exception e) {
				...
				try {
					...
					throw new ApplicationException("插入减免申报表失败，数据库操作失败，请您找管理员联系！");
				}
			}
	...
	private Object doDelete(VOPackage vo) throws BaseException {
		...
		try {
			...
			this.insertJmJb(jmje, czzssdsNbForm, da, ud, djxx, now, now, sbrq,skssjsrq, skssksrq, nd);
			...
			// 将数据库中的数据翻译成页面所需数据格式
			int[] arrs = { 1, 32 };
			czzssdsNbForm.setQysdsnbList(null);
