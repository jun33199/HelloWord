...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.srbjrqy.web;
...
extends SmsbAction
{
	...
	throws
	BaseException
	{   SrbjrqyForm srbjrqyForm=new SrbjrqyForm();
		this.getBaseForm(request,srbjrqyForm);
		VOPackage vo = new VOPackage();
		vo.setData(srbjrqyForm);
		vo.setUserData(userData);
		vo.setAction(CodeConstant.SMSB_SHOWACTION);
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_SRBJRQY_PROCESSOR);
		try {
			srbjrqyForm = (SrbjrqyForm) SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), srbjrqyForm);
		} catch (Exception ex) {
			srbjrqyForm.reset(mapping, request);
			// 系统捕捉异常，根据异常类型抛出
	...
	BaseException
	{
		...
		SrbjrqyForm nbForm = (SrbjrqyForm) form;
		VOPackage vo = new VOPackage();
		...
		try
		{
			nbForm = (SrbjrqyForm) SbzlProxy.getInstance().process(vo);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		SrbjrqyForm srbjrqyForm = (SrbjrqyForm)form;
		this.getBaseForm(request,srbjrqyForm);
		srbjrqyForm.setDataList(SfRequestUtil.getValuesToList(request, srbjrqyForm.getSb_columns()));
		VOPackage vo = new VOPackage();
		vo.setData(srbjrqyForm);
		vo.setUserData(userData);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		this.getBaseForm(request,srbjrqyForm);
		srbjrqyForm.setDataList(SfRequestUtil.getValuesToList(request, srbjrqyForm.getSb_columns()));
		request.setAttribute("ActionForm",srbjrqyForm);
		VOPackage vo = new VOPackage();
		vo.setData(srbjrqyForm);
		vo.setUserData(userData);
...
//			调用processor
			srbjrqyForm=(SrbjrqyForm)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), srbjrqyForm);
			if(srbjrqyForm.getCheckList()==null)
			{
				request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
				System.out.println("checked1 passed");
			}else if(srbjrqyForm.getCheckList().size()==0)
			{
				request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
				System.out.println("checked2 passed");
			}else if(srbjrqyForm.getCheckList().size()>0)
			{
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(srbjrqyForm.getCheckList()));
			}	
...
//			srbjrqyForm.reset(mapping, request);
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,SrbjrqyForm form)
	{
		...
		if(baseForm!=null)
		{
			...
			form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_SRBJRQY));
			form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_SRBJRQY));
		}
	}
}