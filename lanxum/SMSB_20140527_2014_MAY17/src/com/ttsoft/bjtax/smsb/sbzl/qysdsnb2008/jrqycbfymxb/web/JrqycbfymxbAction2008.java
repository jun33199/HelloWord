...
public class JrqycbfymxbAction2008 extends 
SmsbAction 
{
	/**
	 * 操作员数据
	...
	BaseException
	{
		...
		JrqycbfymxbForm2008 jrqycbfymxbForm = (JrqycbfymxbForm2008)form;
		userData = this.getUserData(request);
		this.getBaseForm(request, jrqycbfymxbForm);//加入纳税人基本数据
		
		VOPackage vo = new VOPackage();
		vo.setAction(CodeConstant.SMSB_SHOWACTION); //设置数据包操作类型
		vo.setUserData(userData);
		vo.setData(jrqycbfymxbForm); //设置操作数据
		vo.setProcessor(CodeConstant.SBZL_QYSDSNB2008_JRQYCBFYMXB_PROCESSOR2008); //设置PROCESSOR
		try
		{
			jrqycbfymxbForm =(JrqycbfymxbForm2008)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), jrqycbfymxbForm); //查询结果放入request
		}catch(Exception ex)
		{
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		try
		{
			//调用processor
			jrqycbfymxbForm=(JrqycbfymxbForm2008)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), jrqycbfymxbForm); //把查询结果放入request
		}catch(Exception ex)
		{
			//系统捕捉异常，根据异常类型抛出
			throw ExceptionUtil.getBaseException(ex);
		}
		request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "保存成功！"); //返回保存结果信息
		return mapping.findForward("Show");
	}   
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		JrqycbfymxbForm2008 jrqycbfymxbForm = (JrqycbfymxbForm2008)form;
		userData = this.getUserData(request);
		this.getBaseForm(request,jrqycbfymxbForm); //加入纳税人基本信息
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		try
		{
			...
			if(jrqycbfymxbForm.getCheckList()==null){
				...
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsUtil2008.getCheckResults(jrqycbfymxbForm.getCheckList()));
			}
			return mapping.findForward("Show");
		}
		catch (Exception ex)
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,JrqycbfymxbForm2008 form)
	{
		...
		if(baseForm!=null)
		{
			...
			form.setNextTableURL(QysdsUtil2008.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_2008_2_2));
			form.setPreviousTableURL(QysdsUtil2008.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_2008_2_2));
		}
	}
