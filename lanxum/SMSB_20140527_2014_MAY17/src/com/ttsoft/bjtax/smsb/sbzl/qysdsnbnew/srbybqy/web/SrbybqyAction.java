...
public class SrbybqyAction extends 
   SmsbAction 
   {
	/**
     * 操作员数据
    ...
    public ActionForward doShow (ActionMapping mapping,
        ...
        BaseException
    {
        SrbybqyForm srbybqyForm = (SrbybqyForm)form;
        userData = this.getUserData(request);
        this.getBaseForm(request, srbybqyForm); //加入纳税人基本数据
        VOPackage vo = new VOPackage();
		vo.setAction(CodeConstant.SMSB_SHOWACTION); //设置操作类型
		vo.setUserData(userData);
		vo.setData(srbybqyForm); //设置操作数据
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_SRBYBQY_PROCESSOR); //设置PROCESSOR
		try
		{
			srbybqyForm =(SrbybqyForm)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), srbybqyForm); //把查询结果放入REQUEST
		}catch(Exception ex)
		{
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
     ...
     SrbybqyForm srbybqyForm = (SrbybqyForm)form;
     userData = this.getUserData(request);
	  String[] sb_columns = srbybqyForm.getSb_columns();
	  srbybqyForm.setSrbybqy(SfRequestUtil.getValuesToList(request, sb_columns)); //设置页面数据
	  this.getBaseForm(request, srbybqyForm); //加入纳税人基本信息
	  request.setAttribute(mapping.getAttribute(), srbybqyForm);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		if(srbybqyForm.getCheckList()==null){
			request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
		}else if(srbybqyForm.getCheckList().size()==0){
			request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
		}else if(srbybqyForm.getCheckList().size()>0){
			request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(srbybqyForm.getCheckList()));
		}
		return mapping.findForward("Show");
   	  }catch(Exception ex)
   	  {
//   		系统捕捉异常，根据异常类型抛出
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,SrbybqyForm form)
	{
		...
		if(baseForm!=null)
		{
			...
			form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_SRBYBQY));
			form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_SRBYBQY));
		}
	}
