...
extends SmsbAction
{
	...
	BaseException
	{
		HzssmxbForm hzssmxbForm = (HzssmxbForm)form;
		this.getBaseForm(request,hzssmxbForm);
		System.out.println(hzssmxbForm.getCkzd());
		VOPackage vo = new VOPackage();
		vo.setData(hzssmxbForm);
		vo.setUserData(userData);
		vo.setAction(CodeConstant.SMSB_SHOWACTION);
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_HZSSMXB_PROCESSOR);
		try {
			hzssmxbForm = (HzssmxbForm) SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), hzssmxbForm);
		} catch (Exception ex) {
			hzssmxbForm.reset(mapping, request);
			// 系统捕捉异常，根据异常类型抛出
	...
	BaseException
	{
		...
		HzssmxbForm nbForm = (HzssmxbForm) form;
		...
		try
		{
			nbForm = (HzssmxbForm) SbzlProxy.getInstance().process(vo);
...
//		-------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		HzssmxbForm hzssmxbForm = (HzssmxbForm)form;
		this.getBaseForm(request,hzssmxbForm);
		hzssmxbForm.setDataList(SfRequestUtil.getValuesToList(request, hzssmxbForm.getSb_columns()));
		VOPackage vo = new VOPackage();
		vo.setData(hzssmxbForm);
		vo.setUserData(userData);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		try
		{
			...
			hzssmxbForm=(HzssmxbForm)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), hzssmxbForm);
			if(hzssmxbForm.getCheckList()==null)
			{
				request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
			}else if(hzssmxbForm.getCheckList().size()==0)
			{
				request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
			}else if(hzssmxbForm.getCheckList().size()>0)
			{
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(hzssmxbForm.getCheckList()));
			}
		}
		catch (Exception ex)
		{
...
//			hzssmxbForm.reset(mapping, request);
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,HzssmxbForm form)
	{
		userData = this.getUserData(request);
		...
		if(baseForm!=null)
		{
			...
			if("no".equals(QysdsNewUtil.isLastTable(baseForm,CodeConstant.TABLE_ID_HZSSMXB))){
				form.setIsLastTable("no");
				form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_HZSSMXB));
			}else{
				form.setIsLastTable("yes");
			}
			form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_HZSSMXB));
