...
public class JwsdmxbAction2008 extends SmsbAction {
	/**
	 * 操作员数据
	 */
	...
	public ActionForward doShow(ActionMapping mapping, ActionForm form,
		...
		JwsdmxbForm2008 jwsdmxbForm = (JwsdmxbForm2008) form;
		...
		vo.setProcessor("com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.jwsdmxb.processor.JwsdmxbProcessor2008");
		vo.setUserData(userData);
		try {

			// 调用processor
			jwsdmxbForm = (JwsdmxbForm2008) SbzlProxy.getInstance().process(vo);
	...
	BaseException
	{
		...
		JwsdmxbForm2008 jwsdsmxbForm = (JwsdmxbForm2008)form;
		...
		vo.setUserData(userData);
		vo.setProcessor("com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.jwsdmxb.processor.JwsdmxbProcessor2008"); //设置processor
		try
		{
			//调用processor
			jwsdsmxbForm =(JwsdmxbForm2008)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), jwsdsmxbForm);
			...
			if(jwsdsmxbForm.getCheckList()==null){
				...
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsUtil2008.getCheckResults(jwsdsmxbForm.getCheckList()));
			}
			return mapping.findForward("Show");
		}
		catch (Exception ex)
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request, JwsdmxbForm2008 form) {
		...
		if (baseForm != null) {
			...
			form.setNextTableURL(QysdsUtil2008.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_2008_6));
			form.setPreviousTableURL(QysdsUtil2008.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_2008_6));
		}
	}
}