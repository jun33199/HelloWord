...
public class NstzmxzjbAction extends 
   SmsbAction 
   {
	/**
     * 操作员数据
    ...
    public ActionForward doShow (ActionMapping mapping,
        ...
        BaseException
    {
    	NstzmxzjbForm nstzmxzjbForm = (NstzmxzjbForm)form;
    	...
    	vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_NSTZMXZJB_PROCESSOR); //设置Processor
    	vo.setUserData(userData);
    	try
		{
    		nstzmxzjbForm =(NstzmxzjbForm)SbzlProxy.getInstance().process(vo);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
    	...
    	String[] nstzzj_columns = nstzmxzjbform.getNstzzj_columns();   	
    	nstzmxzjbform.setDataList(SfRequestUtil.getValuesToList(request, columns));   //设置固定行数据 	
    	nstzmxzjbform.setNstzzj_List(this.getValuesToList(request, nstzzj_columns));  //设置动态行数据
    	this.getBaseForm(request, nstzmxzjbform); //加入纳税人基本数据
    	...
    	vo.setData(nstzmxzjbform); //设置操作数据
    	vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_NSTZMXZJB_PROCESSOR); //PROCESSOR
    	vo.setUserData(userData);
    	try
    	{
    		 //调用processor
			...
			nstzmxzjbform=(NstzmxzjbForm)SbzlProxy.getInstance().process(vo);
			 ...
			 request.setAttribute(mapping.getAttribute(), nstzmxzjbform);
				...
				if(nstzmxzjbform.getCheckList()==null){
					...
					request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(nstzmxzjbform.getCheckList()));
				}
				return mapping.findForward("Show");
		}catch(Exception ex)
		{
//			系统捕捉异常，根据异常类型抛出
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,NstzmxzjbForm form)
	{
		...
		if(baseForm!=null)
		{
			...
			form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_NSTZMXZJB));
			form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_NSTZMXZJB));
		}
	}
}