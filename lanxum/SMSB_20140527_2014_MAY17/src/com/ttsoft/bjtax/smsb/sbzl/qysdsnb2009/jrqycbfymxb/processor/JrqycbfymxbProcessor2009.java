...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2009.jrqycbfymxb.processor;
...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnb2009.jrqycbfymxb.web.JrqycbfymxbForm2009;
...
public class JrqycbfymxbProcessor2009
implements Processor 
{
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		JrqycbfymxbForm2009 nbForm = (JrqycbfymxbForm2009) vo.getData(); //获得操作数据
		...
		try
		{
			...
			table.setTableName(CodeConstant.TABLE_NAME_2009_2_2);
			table.setTbblx(Constants.CREPORTS_IBBQLX_TYPE_YEAR);
			report.getTableContentList().put(table.getTableId(), table);
			//获取数据库接口，调用save方法进行数据保存
			IAppAccess iApp = AppAccessFactory.getAInstance(conn,AppAccessFactory.ACCESS_MODEL_APP_QYSDS);
			iApp.querySingleTable(report);
			table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2009_2_2);
			int[] arrs = {1,50};
			nbForm.setDataList(this.translate2Page(QysdsUtil2009.putSpace(table,arrs)));
		}catch(Exception ex)
		{
			ex.printStackTrace();
	...
	private Object doSave(VOPackage vo) throws BaseException
	{
		...
		JrqycbfymxbForm2009 jrqycbfymxbForm = (JrqycbfymxbForm2009)vo.getData();
		Connection con = null;
		try
		{
			//获取数据库连接
			...
			//将ActionForm中的数据结构转换置入数据库接口参数要求的数据结构
			QysdsReportsDeclare report=this.translate2Interface(jrqycbfymxbForm);
	...
	private Object doDelete(VOPackage vo) throws BaseException
	{
		...
		try
		{
			...
			try
			{
				...
				table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2009_2_2);
				int[] arrs = {1,50};
				jrqycbfymxbForm.setDataList(this.translate2Page(QysdsUtil2009.putSpace(table,arrs)));
			}catch(Exception ex)
			{
				ex.printStackTrace();
			}
	...
	private Object doCheck(VOPackage vo)throws BaseException
	{
		...
		try
		{
			...
			jrqycbfymxbForm.setCheckList(listSingle);
			/*如果校验通过，调用接口保存数据*/
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(JrqycbfymxbForm2009 nbForm)
	{
		//企业所得税报表申明对象
		QysdsReportsDeclare report =new QysdsReportsDeclare();
