 ...
 *
 */
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2009.jrqysrmx.processor;
...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnb2009.jrqysrmx.web.JrqysrmxForm;
...
public class JrqysrmxProcessor implements Processor {
	  ...
	  private Object doShow(VOPackage vo) throws BaseException 
	  {
		...
		try {
			...
			table.setTableName(CodeConstant.TABLE_NAME_2009_1_2);
			table.setTbblx(Constants.CREPORTS_IBBQLX_TYPE_YEAR);
			report.getTableContentList().put(table.getTableId(),table);
			// 获取数据库接口，调用query方法进行数据查找
			IAppAccess iApp=AppAccessFactory.getAInstance(conn,AppAccessFactory.ACCESS_MODEL_APP_QYSDS);
			iApp.querySingleTable(report);
			table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2009_1_2);
			int []arra={1,48};
	  ...
	  private Object doDelete(VOPackage vo) throws BaseException 
	  {
		  ...
		  Connection conn = null;
			try {
				...
				int []arra={1,48};
				table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2009_1_2);
				JrqysrmxForm.setDataList(this.translate2Page(QysdsUtil2009.putSpace(table,arra)));
