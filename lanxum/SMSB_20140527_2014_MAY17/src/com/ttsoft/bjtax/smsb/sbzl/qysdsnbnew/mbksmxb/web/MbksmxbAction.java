...
public class MbksmxbAction extends 
   SmsbAction 
   {
	/**
     * 操作员数据
    ...
    public ActionForward doShow (ActionMapping mapping,
        ...
        BaseException
    {
        MbksmxbForm mbksmxbForm = (MbksmxbForm)form;
        ...
        vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_MBKSMXB_PROCESSOR); //设置Processor
        try
        {
        	mbksmxbForm=(MbksmxbForm)SbzlProxy.getInstance().process(vo);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		try
		{
			...
			if(mbksmxbForm.getCheckList()==null){
				...
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(mbksmxbForm.getCheckList()));
			}
			return mapping.findForward("Show");
		}
		catch (Exception ex)
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,MbksmxbForm form)
	{
		...
		if(baseForm!=null)
		{
			...
			form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_MBKSMXB));
			form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_MBKSMXB));
		}
	}
