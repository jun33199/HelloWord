...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.zb.processor;
...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.zb.web.ZbForm;
...
public class ZbProcessor implements Processor {
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		ZbForm zbForm = (ZbForm) vo.getData();
		...
		try {
			...
			QysdsNewUtil.setQysdsReport(report, zbForm);
			// 设置QysdsReportsTableDeclare的基本信息
			QysdsReportsTableDeclare table = new QysdsReportsTableDeclare();
			table.setTableId(CodeConstant.TABLE_ID_ZB);
			...
			table = (QysdsReportsTableDeclare) report.getTableContentList()
					.get(CodeConstant.TABLE_ID_ZB);
			// 将数据库中的数据翻译成页面所需数据格式
			int[] arrs = { 1, 35 };
			zbForm.setDataList(this.translate2Page(QysdsNewUtil.putSpace(table,
					arrs)));
		} catch (Exception ex) {
			// 抛出异常
			ex.printStackTrace();
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(ZbForm form) {
	 ...
	 *            当前时间
	 */
	private void putJm(String jmje, QysdsSet sdsInfo, ZbForm form,
