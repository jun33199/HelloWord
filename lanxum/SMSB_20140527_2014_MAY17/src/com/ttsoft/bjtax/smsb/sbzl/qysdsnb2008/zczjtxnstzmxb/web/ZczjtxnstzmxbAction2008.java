...
public class ZczjtxnstzmxbAction2008
extends SmsbAction
{
	/**
	 * 操作员数据
	...
	BaseException
	{
		ZczjtxnstzmxbForm2008 zcmxbForm = (ZczjtxnstzmxbForm2008)form;
		this.getBaseForm(request,zcmxbForm);
		VOPackage vo = new VOPackage();
		vo.setData(zcmxbForm);
		vo.setUserData(userData);
		vo.setAction(CodeConstant.SMSB_SHOWACTION);
		vo.setProcessor(CodeConstant.SBZL_QYSDSNB2008_ZCZJTXNSTZMXB_PROCESSOR2008);
		try {
			zcmxbForm = (ZczjtxnstzmxbForm2008) SbzlProxy.getInstance().process(vo);
	...
	BaseException
	{
		...
		ZczjtxnstzmxbForm2008 nbForm = (ZczjtxnstzmxbForm2008) form;
		VOPackage vo = new VOPackage();
		...
		try
		{
			nbForm = (ZczjtxnstzmxbForm2008) SbzlProxy.getInstance().process(vo);
...
//		-------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		try
		{
			...
			{
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsUtil2008.getCheckResults(zcmxbForm.getCheckList()));
			}	
...
//			zcmxbForm.reset(mapping, request);
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request,ZczjtxnstzmxbForm2008 form)
	{
		...
		if(baseForm!=null)
		{
			...
			form.setNextTableURL(QysdsUtil2008.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_2008_9));
			form.setPreviousTableURL(QysdsUtil2008.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_2008_9));
		}
	}
}