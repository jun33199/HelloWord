...
SmsbAction 
{
	...
	BaseException
	{
		...
		this.getBaseForm(request, cbmxbjrqyForm);//加入纳税人基本数据
		
		VOPackage vo = new VOPackage();
		vo.setAction(CodeConstant.SMSB_SHOWACTION); //设置数据包操作类型
		vo.setUserData(userData);
		vo.setData(cbmxbjrqyForm); //设置操作数据
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_CBMXBJRQY_PROCESSOR); //设置PROCESSOR
		try
		{
			cbmxbjrqyForm =(CbmxbjrqyForm)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), cbmxbjrqyForm); //查询结果放入request
		}catch(Exception ex)
		{
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		try
		{
			//调用processor
			cbmxbjrqyform=(CbmxbjrqyForm)SbzlProxy.getInstance().process(vo);
			request.setAttribute(mapping.getAttribute(), cbmxbjrqyform); //把查询结果放入request
		}catch(Exception ex)
		{
			//系统捕捉异常，根据异常类型抛出
			throw ExceptionUtil.getBaseException(ex);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		CbmxbjrqyForm cbmxbjrqyform = (CbmxbjrqyForm)form;
		userData = this.getUserData(request);
		this.getBaseForm(request,cbmxbjrqyform); //加入纳税人基本信息
