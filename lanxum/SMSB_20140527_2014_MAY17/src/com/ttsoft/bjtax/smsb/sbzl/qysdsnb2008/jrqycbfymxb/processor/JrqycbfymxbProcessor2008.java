...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.jrqycbfymxb.processor;
...
public class JrqycbfymxbProcessor2008
implements Processor 
{
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		JrqycbfymxbForm2008 nbForm = (JrqycbfymxbForm2008) vo.getData(); //获得操作数据
		...
		try
		{
			...
			table.setTableName(CodeConstant.TABLE_NAME_2008_2_2);
			table.setTbblx(Constants.CREPORTS_IBBQLX_TYPE_YEAR);
			report.getTableContentList().put(table.getTableId(), table);
			//获取数据库接口，调用save方法进行数据保存
			IAppAccess iApp = AppAccessFactory.getAInstance(conn,AppAccessFactory.ACCESS_MODEL_APP_QYSDS);
			iApp.querySingleTable(report);
			table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2008_2_2);
			int[] arrs = {1,50};
	...
	private Object doSave(VOPackage vo) throws BaseException
	{
		...
		JrqycbfymxbForm2008 jrqycbfymxbForm = (JrqycbfymxbForm2008)vo.getData();
		Connection con = null;
		try
		{
			//获取数据库连接
	...
	private Object doDelete(VOPackage vo) throws BaseException
	{
		...
		try
		{
			...
			try
			{
				...
				table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2008_2_2);
				int[] arrs = {1,50};
				jrqycbfymxbForm.setDataList(this.translate2Page(QysdsUtil2008.putSpace(table,arrs)));
			}catch(Exception ex)
			{
				ex.printStackTrace();
			}
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(JrqycbfymxbForm2008 nbForm)
	{
		//企业所得税报表申明对象
		QysdsReportsDeclare report =new QysdsReportsDeclare();
