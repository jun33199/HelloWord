...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.gg.web;
...
public class GgAction2008 extends SmsbAction {
	/**
	 * 操作员数据
	 */
	...
	{
		// 获取年报数据,加注基本信息
		GgForm2008 ggForm = (GgForm2008) form;
		this.getBaseForm(request, ggForm);
		VOPackage vo = new VOPackage();
		vo.setData(ggForm);
		vo.setUserData(userData);
		vo.setAction(CodeConstant.SMSB_SHOWACTION);
		vo.setProcessor("com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.gg.processor.GgProcessor2008");
		try {
			// 调用process
			ggForm = (GgForm2008) SbzlProxy.getInstance().process(vo);
...
//		-------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
		...
		try {
			...
			{
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsUtil2008.getCheckResults(ggForm.getCheckList()));
			}
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request, GgForm2008 form)
	{
		...
		if (baseForm != null) {
			...
			form.setNextTableURL(QysdsUtil2008.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_2008_8));
			form.setPreviousTableURL(QysdsUtil2008.getTableURL(baseForm,"P_"+CodeConstant.TABLE_ID_2008_8));
		}
	}
}