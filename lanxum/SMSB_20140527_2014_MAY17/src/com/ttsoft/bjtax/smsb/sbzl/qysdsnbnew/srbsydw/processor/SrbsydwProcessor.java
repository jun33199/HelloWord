...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.srbsydw.processor;
...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnbnew.srbsydw.web.SrbsydwForm;
...
public class SrbsydwProcessor implements Processor {
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		SrbsydwForm srbsydwForm = (SrbsydwForm) vo.getData();
		...
		try {
			...
			QysdsNewUtil.setQysdsReport(report, srbsydwForm);
			...
			// 将数据库中的数据翻译成页面所需数据格式
			int[] arrs = { 1, 24 };
			srbsydwForm.setDataList(this.translate2Page(QysdsNewUtil.putSpace(
					table, arrs)));
		} catch (Exception ex) {
			// 抛出异常
			ex.printStackTrace();
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(SrbsydwForm form) {
