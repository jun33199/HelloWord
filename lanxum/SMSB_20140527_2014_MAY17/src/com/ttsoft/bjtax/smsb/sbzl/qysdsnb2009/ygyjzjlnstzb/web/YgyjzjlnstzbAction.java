package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2009.ygyjzjlnstzb.web;
 ...
 * @version 1.0
 */
public class YgyjzjlnstzbAction extends SmsbAction {
    /**
     * 操作员数据
     */
    ...
    public ActionForward doShow(ActionMapping mapping,
            ...
            BaseException {
        YgyjzjlnstzbForm ygyjzjlnstzbForm = (YgyjzjlnstzbForm) form;
        this.getBaseForm(request, ygyjzjlnstzbForm); //加入纳税人基本数据
        userData = this.getUserData(request);

        VOPackage vo = new VOPackage();
        vo.setAction(CodeConstant.SMSB_SHOWACTION); //设置操作类型
        vo.setData(ygyjzjlnstzbForm); //设置操作数据
        vo.setProcessor(CodeConstant.
                        SBZL_QYSDSNB2009_YGYJZJLNSTZB_PROCESSOR2009); //设置Processor
        vo.setUserData(userData);
        try {
            ygyjzjlnstzbForm = (YgyjzjlnstzbForm) SbzlProxy.getInstance().
                               process(
                                       vo);
            request.setAttribute(mapping.getAttribute(), ygyjzjlnstzbForm); //把查询结果放入REQUEST
        } catch (Exception ex) {
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
        ...
        YgyjzjlnstzbForm ygyjzjlnstzbForm = (YgyjzjlnstzbForm) form;
        userData = this.getUserData(request);
        String[] columns = ygyjzjlnstzbForm.getSb_columns();
        ygyjzjlnstzbForm.setDataList(SfRequestUtil.getValuesToList(request,
                columns)); //设置固定行数据
        this.getBaseForm(request, ygyjzjlnstzbForm); //加入纳税人基本数据
        request.setAttribute(mapping.getAttribute(), ygyjzjlnstzbForm);

        VOPackage vo = new VOPackage();
        vo.setAction(CodeConstant.SMSB_SAVEACTION); //设置操作类型
        vo.setData(ygyjzjlnstzbForm); //设置操作数据
        vo.setProcessor(CodeConstant.
                        SBZL_QYSDSNB2009_YGYJZJLNSTZB_PROCESSOR2009); //PROCESSOR
        vo.setUserData(userData);
        try {
            //调用processor
            ygyjzjlnstzbForm = (YgyjzjlnstzbForm) SbzlProxy.getInstance().
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
        ...
        YgyjzjlnstzbForm ygyjzjlnstzbForm = (YgyjzjlnstzbForm) form;
        userData = this.getUserData(request);
        this.getBaseForm(request, ygyjzjlnstzbForm); //加入纳税人基本基本数据
...
//			调用processor
            ...
            if (ygyjzjlnstzbForm.getCheckList() == null) {
                request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
            } else if (ygyjzjlnstzbForm.getCheckList().size() == 0) {
                request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "审核通过！");
            } else if (ygyjzjlnstzbForm.getCheckList().size() > 0) {
...
//			系统捕捉异常，根据异常类型抛出
     ...
     * @return
     */
    private void getBaseForm(HttpServletRequest request, YgyjzjlnstzbForm form) {
        ...
        if (baseForm != null) {
            ...
            form.setNextTableURL(QysdsUtil2009.getTableURL(baseForm,
                    "N_" + CodeConstant.TABLE_ID_2009_7));
            form.setPreviousTableURL(QysdsUtil2009.getTableURL(baseForm,
                    "P_" + CodeConstant.TABLE_ID_2009_7));
        }
    }
}
