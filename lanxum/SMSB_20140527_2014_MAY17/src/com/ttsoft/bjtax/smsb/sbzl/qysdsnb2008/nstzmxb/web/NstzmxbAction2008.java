 ...
 * @version 1.0
 */
public class NstzmxbAction2008 extends SmsbAction {
    /**
     * 操作员数据
     */
    ...
    public ActionForward doShow(ActionMapping mapping,
            ...
            BaseException {
        NstzmxbForm2008 nstzmxzjbForm = (NstzmxbForm2008) form;
        ...
        vo.setProcessor(CodeConstant.SBZL_QYSDSNB2008_NSTZMXB_PROCESSOR2008); //设置Processor
        vo.setUserData(userData);
        try {
            nstzmxzjbForm = (NstzmxbForm2008) SbzlProxy.getInstance().process(
                    vo);
...
//		 -------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
        ...
        vo.setData(nstzmxzjbform); //设置操作数据
        vo.setProcessor(CodeConstant.SBZL_QYSDSNB2008_NSTZMXB_PROCESSOR2008); //PROCESSOR
        vo.setUserData(userData);
        try {
            //调用processor
            nstzmxzjbform = (NstzmxbForm2008) SbzlProxy.getInstance().process(
...
//			系统捕捉异常，根据异常类型抛出
     ...
     * @return
     */
    private void getBaseForm(HttpServletRequest request, NstzmxbForm2008 form) {
        ...
        if (baseForm != null) {
            ...
            form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,
                    "N_" + CodeConstant.TABLE_ID_2008_3));
            form.setPreviousTableURL(QysdsNewUtil.getTableURL(baseForm,
                    "P_" + CodeConstant.TABLE_ID_2008_3));
        }
    }
}
