...
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2013.jbxxb.web;
...
public class JbxxbAction2013 extends SmsbAction {
	...
	public ActionForward doShow(ActionMapping mapping, ActionForm form,
		...
		//获取基本信息
		JbxxbForm2013 jbxxbForm = (JbxxbForm2013) form;
		jbxxbForm.reset(mapping, request);
		userData = this.getUserData(request);	
		VOPackage vo = new VOPackage();
		vo.setAction(CodeConstant.SMSB_SHOWACTION);
		vo.setData(jbxxbForm);
		vo.setProcessor("com.ttsoft.bjtax.smsb.sbzl.qysdsnb2013.jbxxb.processor.JbxxbProcessor2013");
		vo.setUserData(userData);
		try {
			//调用process
			jbxxbForm = (JbxxbForm2013) SbzlProxy.getInstance().process(vo);
	...
	public ActionForward doSave(ActionMapping mapping, ActionForm form,
		...
		request.setAttribute(mapping.getAttribute(), jbxxbForm);
		if(jbxxbForm.getQueryFlag().equals("999")){
			request.setAttribute(CodeConstant.SMSB_SAVE_SUCCESS, "当期申报数据被删除，如有需要请重新录入数据进行保存！");
		}else{
...
/*//    	-------------对数据库操作的Method进行修改，防止刷新或重复提交----------------------------------------
        ...
        try
        {
			...
			jbxxbForm.setSybs(checkBean.getJdlx());
            request.setAttribute(mapping.getAttribute(), jbxxbForm);
            this.getSybsMc(jbxxbForm);
        }
        catch (Exception ex)
	 ...
	 * @param jbxxbForm
	 */
	public void getSybsMc(JbxxbForm2013 jbxxbForm){
		String sybs=jbxxbForm.getSybs();
	 ...
	 * @return
	 */
	private CheckBean setCheckInf(JbxxbForm2013 jbxxbForm)
	{
