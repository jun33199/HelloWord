...
public class HdzssdsjbProcessor implements Processor {
	...
	private Object doQuery(VOPackage vo) throws BaseException {
		...
		try {
			...
			try {
				// 获得企业登记基本信息
				djsj = InterfaceDj.getJBSJ_New(hdzssdsjbForm.getJsjdm(), vo.getUserData());
			} catch (Exception ex1) {
				throw new ApplicationException("没有该纳税人的登记信息或者没有权限查看该纳税人信息！");
			}
			//获取是否是新开户  是：Y 否：N 
			hdzssdsjbForm.setSfxkh(qysdsUtil2014.getSfxkh(nd,djsj,jd));
			//获取税款所属期所在年度上一年度征收方式
			hdzssdsjbForm.setSyndZsfsdm(qysdsUtil2014.getSyndZsfsDm(hdzssdsjbForm.getJsjdm(), nd,jd));
			//获取税款所属期所在年度上一年度的汇算清缴申报信息 主表行9、25 附表五45、46、47
			Map map=qysdsUtil2014.getSyndHsqjSbxx(hdzssdsjbForm.getSyndZsfsdm(),hdzssdsjbForm.getJsjdm(), nd,jd);
			hdzssdsjbForm.setSyndZbh6(map.get("syndZbh6").toString());
			hdzssdsjbForm.setSyndZbh25(map.get("syndZbh25").toString());
			hdzssdsjbForm.setSyndFb5jyjg(map.get("syndFb5jyjg").toString());
			System.out.println("企业征收类型1_"+hdzssdsjbForm.getQyzslx());
			// 税费核定信息
			this.getHdxx(hdzssdsjbForm);
			System.out.println("企业征收类型2_"+hdzssdsjbForm.getQyzslx());
			/* 征收方式 */
			...
			table.setTableId(CodeConstant.TABLE_ID_HDZSSDSJB_2008);
			table.setTableName(CodeConstant.TABLE_NAME_HDZSSDSJB_2008);
			...
			table = (QysdsReportsTableDeclare) report.getTableContentList()
					.get(CodeConstant.TABLE_ID_HDZSSDSJB_2008);
...
//			 /* 征收方式 */
			...
			QysdsReportsDeclare report = this
					.translate2Interface(hdzssdsjbForm,zsfs);
...
//			 * 保存减免税
//			 */
//			List list = hdzssdsjbForm.getQysdsjbList();
//
//			for (int i = 0; i < list.size(); i++) {
//
//				if (i == 12) {
//					// 第十三行的减免税额
//					HashMap map = (HashMap) list.get(12);
//					System.out.println(hdzssdsjbForm.getJsjdm() + "的减免行次:"
//							+ (String) map.get("hc"));
//					System.out.println(hdzssdsjbForm.getJsjdm() + "的减免行次:"
//							+ (String) map.get("lje"));
//
...
//							.getTimestamp(hdzssdsjbForm.getSkssksrq());
//					Timestamp skssjsrq = QysdsNewUtil
//							.getTimestamp(hdzssdsjbForm.getSkssjsrq());
//					Timestamp sbrq = QysdsNewUtil.getTimestamp(hdzssdsjbForm
//							.getSbrq());
//
...
//					this.insertJmJb((String) map.get("lje"), hdzssdsjbForm,
...
//			}
			...
			QysdsReportsTableDeclare table = (QysdsReportsTableDeclare) report
					.getTableContentList().get(CodeConstant.TABLE_ID_HDZSSDSJB_2008);
			...
			hdzssdsjbForm.setQysdsjbList(null);
	 ...
	 * @return 企业所得税报表申明对象
	 */
	private QysdsReportsDeclare translate2Interface(HdzssdsjbForm form,String zsfs) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if ("15".equals(hc)) {
				...
				item_1.setItemID("15");
//				item_1.setItemValue((String) map.get("lje"));
//				System.out.println("保存lje15"+(String) map.get("lje"));
				/*--modified by huohb,2014-07-31--*/
				item_1.setItemValue("0");
				System.out.println("保存lje15"+"0");
				/*--modified end--*/
				item_1.setItemType("11");
				table.getCellContentList().put(item_1.getItemID(), item_1);
			}
	...
	private List translate2Page(QysdsReportsTableDeclare table,String zsfs) {
		...
		map15.put("hc","15");
		map15.put("lje",((QysdsReportsItemDeclare) table.getCellContentList()
				.get("15")).getItemValue());
		pagelist.add(map15);
		//add end 20140610
		map16.put("hc","16");
	 ...
	 *            年度
	 */
//	private void insertJmJb(String jmje, HdzssdsjbForm form,
...
//				this.insertJmJb(jmje, hdzssdsjbForm,
//						da, ud, djxx, now, now, sbrq, skssjsrq, skssksrq,
//						nd);
