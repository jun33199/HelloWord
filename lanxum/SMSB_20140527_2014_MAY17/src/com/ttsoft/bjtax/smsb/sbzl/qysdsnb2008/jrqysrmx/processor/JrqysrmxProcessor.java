 ...
 *
 */
package com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.jrqysrmx.processor;
...
import com.ttsoft.bjtax.smsb.sbzl.qysdsnb2008.jrqysrmx.web.JrqysrmxForm;
 ...
 * @version 1.1
 */
public class JrqysrmxProcessor implements Processor {
	  /**
	   * 实现Processor接口
	   * @param vo 业务参数
	  ...
	  {
	    JrqysrmxForm nbForm = (JrqysrmxForm) vo.getData();
		...
		try {
			...
			table.setTableId(CodeConstant.TABLE_ID_2008_1_2);
			table.setTableName(CodeConstant.TABLE_NAME_2008_1_2);
			table.setTbblx(Constants.CREPORTS_IBBQLX_TYPE_YEAR);
			report.getTableContentList().put(table.getTableId(),table);
			// 获取数据库接口，调用query方法进行数据查找
			IAppAccess iApp=AppAccessFactory.getAInstance(conn,AppAccessFactory.ACCESS_MODEL_APP_QYSDS);
			iApp.querySingleTable(report);
			table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2008_1_2);
			int []arra={1,48};
	  ...
	  /**
		...
		{
			JrqysrmxForm form = (JrqysrmxForm) vo.getData();
			Connection conn = null;
			try {
				//获取数据库连接
				conn = SfDBResource.getConnection();
...
//				更新审核状态为“保存成功”
		...
		{
			JrqysrmxForm JrqysrmxForm = (JrqysrmxForm) vo.getData();
			Connection conn = null;
			try
	         {
				//创建数据库连接
				...
				//将ActionForm中的数据结构转换置入数据库接口参数要求的数据结构
				QysdsReportsDeclare report=this.translate2Interface(JrqysrmxForm);
				...
				JrqysrmxForm.setCheckList(listSingle);
			}
			catch (Exception ex)
			{ 
				//抛出异常
	  ...
	  private Object doDelete(VOPackage vo) throws BaseException 
	  {
		  ...
		  Connection conn = null;
			try {
				...
				int []arra={1,48};
				table = (QysdsReportsTableDeclare) report.getTableContentList().get(CodeConstant.TABLE_ID_2008_1_2);
				JrqysrmxForm.setDataList(this.translate2Page(QysdsUtil2008.putSpace(table,arra)));
	  ...
	  private void initForm(JrqysrmxForm nbForm) throws BaseException 
	  {
	   }
	  /**
		 * 将ActionForm中的数据结构转换置入数据库接口参数要求的数据结构
	  ...
	  private QysdsReportsDeclare translate2Interface(JrqysrmxForm form){
			//企业所得税报表申明对象
			QysdsReportsDeclare report =new QysdsReportsDeclare();
