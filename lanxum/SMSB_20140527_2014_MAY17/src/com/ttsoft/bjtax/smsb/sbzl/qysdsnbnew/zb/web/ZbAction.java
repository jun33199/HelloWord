...
public class ZbAction extends SmsbAction {
	...
	public ActionForward doShow(ActionMapping mapping, ActionForm form,
		...
		vo.setProcessor(CodeConstant.SBZL_QYSDSNBNEW_ZB_PROCESSOR);
		try {
			// 调用Proxy，执行processor,获取zbForm返回值
			zbForm = (ZbForm) SbzlProxy.getInstance().process(vo);
	...
	public ActionForward doCheck(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if(zbForm.getCheckList()==null){
				...
				request.setAttribute(CodeConstant.CHECK_RESULT_HTML, QysdsNewUtil.getCheckResults(zbForm.getCheckList()));
			}
			return mapping.findForward("Show");
		} catch (Exception ex) {
			// 系统捕捉异常，根据异常类型抛出
	 ...
	 * @return
	 */
	private void getBaseForm(HttpServletRequest request, ZbForm form) {
		// 获取本缴费系统用户信息
		userData = this.getUserData(request);
		...
		if (baseForm != null) {
			...
			form.setNextTableURL(QysdsNewUtil.getTableURL(baseForm,"N_"+CodeConstant.TABLE_ID_ZB));
		}
	}
