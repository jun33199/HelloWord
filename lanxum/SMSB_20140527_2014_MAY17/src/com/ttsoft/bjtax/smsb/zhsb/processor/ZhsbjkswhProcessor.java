...
public class ZhsbjkswhProcessor
    implements Processor
{
  public ZhsbjkswhProcessor()
  {
  }
   ...
   * @throws BaseException 业务异常
   */
  private ZhsbjkswhActionForm doQuery(VOPackage vo) throws BaseException
  {
...
//              + "  ((SUBSTR(ZWBS, 1, 1) <> '0') OR (SUBSTR(ZWBS, 20, 1) <> '0'))) ");
      ...
      //started added by qianchao 2005-12-19
      Debug.out("com.ttsoft.bjtax.smsb.zhsb.processor.ZhsbjkswhProcessor.doQuery time cost:" + (System.currentTimeMillis() - t));
      //ended   added by qianchao 2005-12-19
...
*/
  ...
  private ZhsbjkswhActionForm doCx(VOPackage vo) throws BaseException
  {
    ...
    try
    {
      ...
      //由于数据来源是扣缴所得税,所以扣缴所得税做完缴款书作废以后,要回写sbdb.sb_jl_kjqysds_kjbgb表的sphm为null
      if("1".equals(form.getKjflag())){
    	  if(conn==null){
    		  conn = SfDBResource.getConnection();
    	  }
    	  StringBuffer sql=new StringBuffer();
    	  	sql.append("update sbdb.sb_jl_kjqysds_kjbgb set sphm=null");
			sql.append(" where bgbxh='");
			sql.append(form.getBgbxh()).append("'");
			System.out.println("---sb_jl_kjqysds_kjbgb-----"+sql.toString());
