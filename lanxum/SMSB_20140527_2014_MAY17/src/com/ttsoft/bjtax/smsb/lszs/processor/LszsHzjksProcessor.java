...
import com.ttsoft.framework.util.ListUtils;
import com.ttsoft.framework.util.StringUtil;
import com.ttsoft.framework.util.VOPackage;

/**
 * <p>Title: 北京地税核心征管系统－－上门申报</p>
 * <p>Description: 零散征收完税证汇总成缴款书。汇总申报缴款单号，汇总缴款书</p>
 * @author 开发六组－－陆峰
 * @version 1.1
 */
public class LszsHzjksProcessor
    implements Processor
{
	//每张票中的最大预算科目
    private static final int EVE_PAGE_YSKM = 14;
	  
    public LszsHzjksProcessor ()
    {
    }
        ...
        throws BaseException
    {
        ...
        //String krrq = String.valueOf(nowTime).substring(0, 10);

        LszsHzjksForm pf = new LszsHzjksForm();
        pf = (LszsHzjksForm) vo.getData();
        //汇总日期范围，缺省是汇总当天的信息
        ...
        throws BaseException
    {
        ...
        try
        {
            ...
            List tempList = da.query(Lswszsbhz.class, tempVector);

            for (int i = 0; i < tempList.size(); i++)
            {
                Lswszsbhz hz = (Lswszsbhz) tempList.get(i);
                ...
                //显示的缴款凭证号，只显示前15位
                //if (strJkpzh.length()>15) map.put("jkpzh",strJkpzh.substring(0,15));
        ...
        throws BaseException
    {
    	...
    	try{
    		...
    		System.out.println("LszsHzjksProcessor.doFp##################list.size()="+list.size());
    	   if(list.size()==0){
    	...
    	LszsHzjksForm pf = (LszsHzjksForm) vo.getData();
    	return pf;
    }
	...
	private void recursionFp(List list,VOPackage vo)  throws BaseException{
		try{
			...
			System.out.println("LszsHzjksProcessor.recursionFp##################list.size()="+list.size());
            //有需要汇总的完税证
			if(list.size()>0){
				    List xhzWszxhList=getXhzWszxhList(list);
				    System.out.println("LszsHzjksProcessor.recursionFp##################xhzWszxhListt.size()="+xhzWszxhList.size());
                	//取得本次需要汇总的完税证号集合 sql
                	String hzsql=getOracleSQLIn(xhzWszxhList,1000,"a.wszxh"," in");
                	System.out.println("LszsHzjksProcessor.recursionFp##################hzsql="+hzsql);
                	//进行汇总操作
                	doSave(vo,hzsql);
                	//取得下一次完税证和预算科目代码List
                	List nextList=getWszhAndYskmdms(vo);
                	System.out.println("LszsHzjksProcessor.recursionFp##################nextlist()="+nextList.size());
                	//进行下一次分票操作（递归调用）
                	//LszsHzjksForm pf = (LszsHzjksForm) vo.getData();
                	if(nextList.size()>0){
     ...
     */
	 private List getXhzWszxhList(List list) throws BaseException{
		 ...
		 for(int i=0;i<list.size();i++){
			  ...
			  //System.out.println("LszsHzjksProcessor.getXhzWszxhList##################wszxh="+wszxh);
			  //预算科目代码List
			  List yskmdmList=(List)((HashMap)list.get(i)).get(wszxh);
			  //System.out.println("LszsHzjksProcessor.getXhzWszxhList##################yskmdmList.size()="+yskmdmList.size());
    ...
    private List getWszhAndYskmdms(VOPackage vo) throws BaseException{
    	...
    	try
         {
            ...
            System.out.println("LszsHzjksProcessor.getWszhAndYskmdms##########sql="+strSql); 		
             ...
             while (rs.next())
             {
                 ...
                 " from sbdb.sb_jl_lsswszmx b " +
                 " where " +
                 " b.jsjdm='"+ rs.getString("jsjdm")+"' and b.wszxh='"+rs.getString("wszxh")+"'"; 
                 //System.out.println(reList.size()+"LszsHzjksProcessor.getWszhAndYskmdms##########sql="+strSql);
        ...
        {
    		 ex.printStackTrace();
    		 throw new ApplicationException("查询完税证和预算科目出错！");
        }
        finally
        {
            SfDBResource.freeConnection(conn);
   ...
   return reList;
        ...
        throws BaseException
    {
        ...
        //定义Ormap
        Lsswszmx wszmx = new Lsswszmx();
        Lsswszz wszz = new Lsswszz();
        ...
        try
        {
            ...
            /*strSql = "select distinct b.szsmdm,b.jsjdm," +
                     ...
                     " from sbdb.sb_jl_lsswszz a, sbdb.sb_jl_lsswszmx b " +
                     " where a.ndzb = b.ndzb and a.wszh = b.wszh " +
                     " and a.qxdm='" + qxdm + "' " + timeCon +
                     " and a.clbjdm = '" + CodeConstant.SMSB_WSZ_PRINT + "'" + //已打印
                     " and (a.sbhzdh = '' or a.sbhzdh is null) " + whereCon +
                     " and a.jsjdm=b.jsjdm and a.wszxh=b.wszxh " +
                " group by b.szsmdm, b.jsjdm, b.skssksrq, b.skssjsrq,b.szdm " +
                " order by b.szsmdm asc";
                */
            //调整税款所属日期：在相同税种、税目、预算科目代码下，汇总时间根据税种修改为税款所属开始日期为最小日期，税款所属截止日期为最大日期  Add by Junbing TU/Xi Chen  @@2012-07-13  
            strSql = "with t as (select b.szsmdm,b.jsjdm," +
            		 ...
            		 " and a.clbjdm = '" + CodeConstant.SMSB_WSZ_PRINT + "'" + //已打印
            		 " and (a.sbhzdh = '' or a.sbhzdh is null) " + whereCon +
            		 " and a.jsjdm=b.jsjdm and a.wszxh=b.wszxh"+insql+" ) " +
            ...
            System.out.println("LszsHzjksProcessor.doSave##########sql="+strSql); 		
            ...
            try
            {
                ...
                else
                {
                    ...
                    //补充主表信息//Modified by lufeng 2003-12-09
...
//                    }
                    ...
                    //数据来源，零散征收
                    sbjkzb.setSjly(CodeConstant.SMSB_SJLY_LSHZ);
                    //税款类型代码
            ...
            //获取申报汇总单号
            String sbhzdh = jksUtil.getSequenceOfSbhzd(conn);//pf.getSbhzdh();// jksUtil.getSequenceOfSbhzd(conn); 
            ...
            { //一票一税
                for (int i = 0; i < sbjkzbList.size(); i++)
                {
                    ...
                    try
                    {
                        hzTmpList = (List) updateHZ(da, sbjkzb, sbhzdh, nowTime,
                            pf);
                    }
                    catch (BaseException ex2)
                    {
                        throw ExceptionUtil.getBaseException(ex2);
                    }
            ...
            { //一票多税
                ...
                for (int i = 0; i < sbjkzbList.size(); i++)
                {
                    ...
                    for (int j = 0; j < onePageList.size(); j++)
                    {
                        ...
                        {
                            throw ExceptionUtil.getBaseException(ex3);
                        }
            ...
            //回填汇总单号到
            strSql = "update sbdb.sb_jl_lsswszz a set a.sbhzdh='" + sbhzdh
                     + "'," +
                     " a.lrrq=sysdate " +
            ...
            //pf.getDataList().addAll(detailList);
            
            pf.setSbhzdh(sbhzdh);
            
            //   System.out.println("1hjje####="+hjje);
              // System.out.println("1pf.getHjje()####="+pf.getHjje());
               //System.out.println("1StringUtil.getDouble( pf.getHjje(),0)####="+StringUtil.getDouble( pf.getHjje(),0));
            
            //pf.setHjje(String.valueOf(StringUtil.getDouble( pf.getHjje(),0)+hjje));
            ...
            System.out.println("LszsHzjksProcessor.doSave##########sbjkzb.getSbbh()="+sbjkzb.getSbbh()); 	
    ...
    private List updateHZ (SfDBAccess sfda, Sbjkzb sbjkzb, String sbhzdh,
                           Timestamp nowTime, LszsHzjksForm pf)
        throws
        BaseException
    {

        List tempList = new ArrayList();
        Lswszsbhz hz = new Lswszsbhz();
        try
        {
            //sbjkzb = (Sbjkzb) dclInfo.getSbjkzb();
            hz.setSbhzdh(sbhzdh);
