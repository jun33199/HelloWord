...
public class Basx12Action extends SmsbAction{
	/**
	 * 操作员数据
	 */
	...
	throws BaseException {
		Basx12Form basx12Form = (Basx12Form) form;
		//设置计算机代码和备案年度
		if(!"".equals(basx12Form.getAdd_jsjdm())){
			basx12Form.setJsjdm(basx12Form.getAdd_jsjdm());
		}
		if(!"".equals(basx12Form.getAdd_band())){
			basx12Form.setBand(basx12Form.getAdd_band());
			//设置起始和截止日期
			basx12Form.setQsrq(basx12Form.getBand()+"-01-01");
			basx12Form.setJzrq(basx12Form.getBand()+"-12-31");
			basx12Form.setMr_band(basx12Form.getBand());
		}
		userData = this.getUserData(request);
		
		basx12Form.setMr_lrr(userData.getYhid());
		SimpleDateFormat sf=new SimpleDateFormat("yyyyMMdd");
		basx12Form.setMr_lrrq(sf.format(new Date()));
		
		VOPackage vo = new VOPackage();
		vo.setUserData(userData);
		vo.setData(basx12Form);
		vo.setProcessor(CodeConstant.SMSB_QYSDSJMSBA_BASX12_PROCESSOR);
		if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_ADD).equals(basx12Form.getCzlx())){
			vo.setAction(CodeConstant.SMSB_SHOWACTION);
		}else{
			vo.setAction(CodeConstant.SMSB_QUERYACTION);
		}
		try {
			basx12Form = (Basx12Form) SbzlProxy.getInstance().process(vo);
			//设置资源利用类型下拉框
           //Basx12ActionHelper.initialPageSelectItem(request,basx12Form.getZyzhlyzlList());
			request.setAttribute(mapping.getAttribute(), basx12Form);
		} catch (Exception ex) {
			// 系统捕捉异常，根据异常类型抛出
			throw ExceptionUtil.getBaseException(ex);
		}
		if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_CHECK).equals(basx12Form.getCzlx())){
			return mapping.findForward("Check");
		}else if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_VIEW).equals(basx12Form.getCzlx())){
	...
	public ActionForward doJssq(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			//调用校验
			CheckBean checkBean =QysdsUtil.setCheckInf(basx12Form);	
        ...
        if("2".equals(basx12Form.getCzlx())){
            ...
            HttpServletRequest request,HttpServletResponse response)throws BaseException
	{
		...
		String url = ForwardPath.PAGE_JMSBAJL_PRINT+"&basqwsh="+basx12Form.getBasqwsh()+"&czlx="+basx12Form.getCzlx();
            ...
            HttpServletRequest request,HttpServletResponse response)throws BaseException
	{
		...
		return doZfbg(mapping,basx12Form,request,response);
	}   
