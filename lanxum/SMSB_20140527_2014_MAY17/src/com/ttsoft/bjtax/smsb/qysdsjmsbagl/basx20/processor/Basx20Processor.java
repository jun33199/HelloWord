...
public class Basx20Processor implements Processor{
	...
	private Object doShow(VOPackage vo) throws BaseException {
		...
		try {
			...
			ex.printStackTrace();
			throw new ApplicationException("数据库查询记录失败！" + basx20Form.getJsjdm()	+ ":" + ex.getMessage());
		} finally {		
	...
	private Object doSave(VOPackage vo) throws BaseException {
		...
		try {
			...
			ex.printStackTrace();
			throw new ApplicationException("数据库保存记录失败！" + basx20Form.getJsjdm()	+ ":" + ex.getMessage());
		} finally {		
			SfDBResource.freeConnection(conn);
		}
		return basx20Form;
...
//			ps.execute();
			...
			ex.printStackTrace();
			throw new ApplicationException("数据库审核记录失败！" + basx20Form.getJsjdm()	+ ":" + ex.getMessage());
		} finally {
			SfDBResource.freeConnection(conn);
		}
		return basx20Form;
