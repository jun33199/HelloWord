...
public class Basx08Action extends SmsbAction{
	/**
	 * 操作员数据
	 */
	...
	throws BaseException {
		Basx08Form basx08Form = (Basx08Form) form;
		//设置计算机代码和备案年度
		if(!"".equals(basx08Form.getAdd_jsjdm())){
			basx08Form.setJsjdm(basx08Form.getAdd_jsjdm());
		}
		if(!"".equals(basx08Form.getAdd_band())){
			basx08Form.setBand(basx08Form.getAdd_band());
			//设置起始和截止日期
			basx08Form.setQsrq(basx08Form.getBand()+"-01-01");
			basx08Form.setJzrq(basx08Form.getBand()+"-12-31");
			basx08Form.setMr_band(basx08Form.getBand());
		}
		userData = this.getUserData(request);
		
		basx08Form.setMr_lrr(userData.getYhid());
		SimpleDateFormat sf=new SimpleDateFormat("yyyyMMdd");
		basx08Form.setMr_lrrq(sf.format(new Date()));
		
		VOPackage vo = new VOPackage();
		vo.setUserData(userData);
		vo.setData(basx08Form);
		vo.setProcessor(CodeConstant.SMSB_QYSDSJMSBA_BASX08_PROCESSOR);
		if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_ADD).equals(basx08Form.getCzlx())){
			vo.setAction(CodeConstant.SMSB_SHOWACTION);
		}else{
			vo.setAction(CodeConstant.SMSB_QUERYACTION);
		}
		try {
			basx08Form = (Basx08Form) SbzlProxy.getInstance().process(vo);
			//设置资源利用类型下拉框
			Basx08ActionHelper.initialPageSelectItem(request,basx08Form.getGxjslyList());
			request.setAttribute(mapping.getAttribute(), basx08Form);
		} catch (Exception ex) {
			// 系统捕捉异常，根据异常类型抛出
			throw ExceptionUtil.getBaseException(ex);
		}
		if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_CHECK).equals(basx08Form.getCzlx())){
			return mapping.findForward("Check");
		}else if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_VIEW).equals(basx08Form.getCzlx())){
	...
	public ActionForward doJssq(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			//调用校验
			CheckBean checkBean =QysdsUtil.setCheckInf(basx08Form);	
		...
		request.setAttribute(mapping.getAttribute(), basx08Form);
		if("2".equals(basx08Form.getCzlx())){
            ...
            HttpServletRequest request,HttpServletResponse response)throws BaseException
	{
		...
		String url = ForwardPath.PAGE_JMSBAJL_PRINT+"&basqwsh="+basx08Form.getBasqwsh()+"&czlx="+basx08Form.getCzlx();
            ...
            HttpServletRequest request,HttpServletResponse response)throws BaseException
	{
		...
		return doZfbg(mapping,basx08Form,request,response);
	}   
