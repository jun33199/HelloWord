...
public class Basx07Action extends SmsbAction{
	/**
	 * 操作员数据
	 */
	...
	throws BaseException {
		Basx07Form basx07Form = (Basx07Form) form;
		//设置计算机代码和备案年度
		if(!"".equals(basx07Form.getAdd_jsjdm())){
			basx07Form.setJsjdm(basx07Form.getAdd_jsjdm());
		}
		if(!"".equals(basx07Form.getAdd_band())){
			basx07Form.setBand(basx07Form.getAdd_band());
			//设置起始和截止日期
			basx07Form.setQsrq(basx07Form.getBand()+"-01-01");
			basx07Form.setJzrq(basx07Form.getBand()+"-12-31");
			basx07Form.setMr_band(basx07Form.getBand());
		}
		userData = this.getUserData(request);
		
		basx07Form.setMr_lrr(userData.getYhid());
		SimpleDateFormat sf=new SimpleDateFormat("yyyyMMdd");
		basx07Form.setMr_lrrq(sf.format(new Date()));
		
		VOPackage vo = new VOPackage();
		vo.setUserData(userData);
		vo.setData(basx07Form);
		vo.setProcessor(CodeConstant.SMSB_QYSDSJMSBA_BASX07_PROCESSOR);
		if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_ADD).equals(basx07Form.getCzlx())){
			vo.setAction(CodeConstant.SMSB_SHOWACTION);
		}else{
			vo.setAction(CodeConstant.SMSB_QUERYACTION);
		}
		try {
			basx07Form = (Basx07Form) SbzlProxy.getInstance().process(vo);
			//设置资源利用类型下拉框
			Basx07ActionHelper.initialPageSelectItem(request,basx07Form.getJszrlxList());
			request.setAttribute(mapping.getAttribute(), basx07Form);
		} catch (Exception ex) {
			// 系统捕捉异常，根据异常类型抛出
			throw ExceptionUtil.getBaseException(ex);
		}
		if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_CHECK).equals(basx07Form.getCzlx())){
			return mapping.findForward("Check");
		}else if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_VIEW).equals(basx07Form.getCzlx())){
	...
	public ActionForward doJssq(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			//调用校验
			CheckBean checkBean =QysdsUtil.setCheckInf(basx07Form);	
        ...
        if("2".equals(basx07Form.getCzlx())){
            ...
            HttpServletRequest request,HttpServletResponse response)throws BaseException
	{
		...
		String url = ForwardPath.PAGE_JMSBAJL_PRINT+"&basqwsh="+basx07Form.getBasqwsh()+"&czlx="+basx07Form.getCzlx();
            ...
            HttpServletRequest request,HttpServletResponse response)throws BaseException
	{
		...
		return doZfbg(mapping,basx07Form,request,response);
	}   
