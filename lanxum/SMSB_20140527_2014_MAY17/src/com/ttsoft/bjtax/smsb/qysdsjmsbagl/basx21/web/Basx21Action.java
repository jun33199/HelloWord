...
public class Basx21Action extends SmsbAction
{
								...
								HttpServletResponse response)	throws BaseException 
	{
		...
		{		
			basx21Form.setJsjdm(basx21Form.getAdd_jsjdm());				//设置计算机代码和备案年度
		}
		if(!"".equals(basx21Form.getAdd_band()))
		{
			basx21Form.setBand(basx21Form.getAdd_band());				//设置起始和截止日期
		...
		basx21Form.setMr_lrr(userData.getYhid());						//录入人
		SimpleDateFormat sf=new SimpleDateFormat("yyyyMMdd");
		basx21Form.setMr_lrrq(sf.format(new Date()));					//录入时间
		...
		if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_CHECK).equals(basx21Form.getCzlx())){
			...
			if(String.valueOf(CodeConstant.QYSDSJMSBAJL_CZLX_ADD).equals(basx21Form.getCzlx()))
			{
				basx21Form.setMzqsnd(basx21Form.getBand());									//如果是新增则设置默认免征起始年度为备案年度
			}
									...
									HttpServletResponse response)throws BaseException
		{
			...
			/*--进入逻辑处理--*/
				...
				throw ExceptionUtil.getBaseException(ex);		// 系统捕捉异常，根据异常类型抛出
			}
