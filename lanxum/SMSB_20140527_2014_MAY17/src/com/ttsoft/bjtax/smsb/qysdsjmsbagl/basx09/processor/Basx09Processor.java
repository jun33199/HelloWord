...
//			判断是否已经保存过
			...
			ex.printStackTrace();
			throw new ApplicationException("数据库查询记录失败！" + basx09Form.getJsjdm()	+ ":" + ex.getMessage());
		} finally {		
...
//			更新资料清单表
			...
			ex.printStackTrace();
			throw new ApplicationException("数据库保存记录失败！" + basx09Form.getJsjdm()	+ ":" + ex.getMessage());
		} finally {		
			SfDBResource.freeConnection(conn);
		}
		return basx09Form;
...
//			ps.execute();
			...
			ex.printStackTrace();
			throw new ApplicationException("数据库审核记录失败！" + basx09Form.getJsjdm()	+ ":" + ex.getMessage());
		} finally {
			SfDBResource.freeConnection(conn);
		}
		return basx09Form;
