 ...
 */
public class PlcxProcessor2 extends CommonProcessor {
    /**
     * 根据传入数据进行不同的操作.
     *
    ...
    public Object doQueryAll(VOPackage vo) throws BaseException {
        ...
        try {
            ...
            resultList = DAOFactory.getInstance().getPlcxDAO().get2(bo,
                    conditions, conn);
        } catch (Exception ex) {
            // 处理失败信息:控制台 ＋ 日志
            Debug.printException(ex);
            ErrorLog.makeLog(vo.getUserData(), "契税申报征收－PlcxProcessor2，操作出错",
    ...
    public Object doQueryDetail(VOPackage vo) throws BaseException {
        ...
        PlcxMxBo2 bo = (PlcxMxBo2) vo.getData();
        Connection conn = null;
        try {
            conn = QSDBUtil.getConnection();
            resultList = DAOFactory.getInstance().getPlcxDAO().getDetail2(bo,
                    conn);
        } catch (Exception ex) {
            // 处理失败信息:控制台 ＋ 日志
            Debug.printException(ex);
