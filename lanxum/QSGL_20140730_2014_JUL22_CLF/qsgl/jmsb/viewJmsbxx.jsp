...
{
	var childWin;
    childWin =  window.open("/qsgl/jmsb/printJmsbxx.do?sbbh="+document.forms[0].sbbh.value,"print","toolbar=no,location=yes,status=no,menubar=yes,scrollbars=yes,resizable=yes,width=640,height=480");
	if (childWin) {
	    doSubmitForm('Return');
	};
}
...
{
    var childWin;
    childWin = window.open("/qsgl/jmsb/printJmsbHdtzs.do?sbbh="+document.forms[0].sbbh.value,"print","toolbar=no,location=yes,status=no,menubar=yes,scrollbars=yes,resizable=yes,width=640,height=480");
	if (childWin) {
        doSubmitForm('Return');
	};
}
...
<jsp:include page="/include/Header.jsp" flush="true">
	<jsp:param name="subtitle" value="契税减免申报>北京市地方税务局契税减免申报显示表" />
	<jsp:param name="helpURL" value="" />
</jsp:include>


<br>
<html:form action="/jmsb/viewJmsbxx.do">
		...
		<TBODY>
			<TR>
				...
				<TABLE align=center cellSpacing=0 class=table-99>
					<TBODY>
						<TR>
							<TD class=1-td1>北京市地方税务局契税减免申报采集表</TD>
						</TR>
						...
						<TR>
							...
							<TABLE border=0 cellSpacing=0 class=tabled-99 width="99%">
								<TBODY>
									<input type="hidden" name="bzqs" value="<%=jmsbxxBo.isBZ()%>">
									<%
							...
							<TABLE border=0 cellSpacing=0 class=tabled-99 width="99%">
								<TBODY>
									...
									.getZtbs())%></DIV>
										...
										<DIV align=left>&nbsp; 处理前上次状态</DIV>
										</TD>
										<TD class=2-td2-t-left width="25%">
										<DIV align=left>&nbsp;<%=SbState.getCancelStateName4Jmsb(jmsbxxBo.getVoSbzb()
							.getZtbs(), jmsbxxBo.getVoSbzb().getBljmsbs())%></DIV>
										</TD>
										<TD class=2-td2-t-center width="20%">
										<DIV align=left>&nbsp; <%
 			if (!jmsbxxBo.getState().equals(Constants.ZB_ZTBS_BC)
 			&& !jmsbxxBo.getState().equals(Constants.ZB_ZTBS_JS_TY)) {
 %> <img alt=撤销返回上次状态 id=rollback name=rollback
											onclick="doSubmitForm('RollBack')"
											onmouseout=MM_swapImgRestore()
											onmouseover="MM_swapImage('rollback','','<%=static_file%>images/q_cxfhsczt2.jpg',1)"
											src="<%=static_file%>images/q_cxfhsczt1.jpg"
											style="CURSOR: hand"> <%
 }
 %>
										</DIV>
							...
							<DIV align=center>
							<%
							if (!jmsbxxBo.getState().equals(Constants.ZB_ZTBS_ZF)) {
							%> <img alt=打印申报表 id=Printsbb name=Printsbb onclick="doPrintSbb()"
								onmouseout=MM_swapImgRestore()
							...
							<%
								...
								;
								if ((jmsbxxBo.getState().equals(Constants.ZB_ZTBS_JS_TY))
										|| (jmsbxxBo.getState().equals(Constants.ZB_ZTBS_JS_BTY))
										|| (jmsbxxBo.getState().equals(Constants.ZB_ZTBS_DY_HD))) {
								//if (!jmsbxxBo.getState().equals(Constants.ZB_ZTBS_BC)) {	
            ...
            src="<%=static_file%>images/xiugai1.jpg" style="CURSOR: hand" width=79>
							...
							<IMG alt=删除 height=22 id=delete name="btnDelete"
								...
								src="<%=static_file%>images/shanchu1.jpg" style="CURSOR: hand"
								width=79>  <%
 }
 %>
